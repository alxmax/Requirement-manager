...
- [~] Internal-consistency lint: flag a requirement whose structured fields moved to a new model (e.g. `depends_on` edge added, cost/number changed) while its prose Contract/AC still assert the superseded model. | lane: feature
      <!-- DEFERRED to v1.19 (Senate 2026-06-19, runs/senate/2026-06-19_224727-reqmap-item4-internal-consistency-lint.json). Naive 'depends_on id absent from prose' check is empirically refuted (78.6% false-positive on this corpus; would also miss the motivating TRIAS case, an added edge). Full prose-parsing needs NLP (out of scope). Intent parked as REQ-INTCONSISTLINT-028 (status: draft) with a redesign brief: baseline-aware detector, reproduce TRIAS in a fixture, FP <10% before promotion. -->
