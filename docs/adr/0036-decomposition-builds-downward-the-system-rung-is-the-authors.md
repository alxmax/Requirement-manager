...
- **Decided:** 2026-09-07, after a review on that date
- **Owner:** Alex
- **Evidence:** every number below was measured by a reviewer on the committed tree (`b08e7af`)
...
Two proposals were put to the review together, because both touch the corpus's shape:
...
Direct precedents, both one day old and cited in the review: the three-levels review of
2026-09-06 (zero engine changes) and the code-rung decomposition review of 2026-09-06 (the
statistical confirmation-sample request still unmet). Over eight resolved reviews of this
repo's corpus shape, six of six that asked "should the engine build new shape machinery"
resolved as "nothing built"; the one engine change adopted was a measured false-positive
reduction to an existing read-only report.

## What the review found

**On A1.** Refused unanimously, on four independent grounds. (1) It reverses a recorded
...
(`test_satisfies_edges_are_never_proposed_or_written`) — the cost lens priced the deletion of a
...
(Purpose, Stress test). (3) It would be the first engine write of `satisfies:` into a file the engine
...
predicate to per-file judgement (Reversibility, which moved from advisory to blocking on this).
(4) It forecloses the measurement: once the engine fills every hole, the rate at which real
corpora lack a top can never be observed, so A2 can never afterwards clear ADR-0016's floor
(Stress test).
...
construction*, an argument a detector that has never fired cannot make (Statistics, answering
the authority-and-precedent objection).
...
today, deterministic, reproduced independently by four reviewers with two tools. B4's headline
figure ("30% of ARCH violate ADR-0025 rule 3") was an instrument reading: five defensible
readings of the rule span 1.2% to 31.3% on one frozen tree, three reviewers got 20, 21 and 24,
and the substantive gap — a parent that does not name a declared child — is **2 of 60**:
`ARCH-CLARIFY-062` missing `REQ-CLARIFY-975` and `ARCH-DESIGN-061` missing `REQ-DESIGN-991`.
B3's instrument is sound (word count, zero cross-implementation variance) but its threshold was
unset: 250 words fires on 8.5% of the corpus, 300 on 4.5%. B5's three under-sized leaves were
the uncommitted residue of the decomposition run made that morning; on the committed tree its n
is 0. B2 has no numeric target and, as the stress test showed, `sync --retire` strips the
loser's tags without repointing them — the "no tag left dangling" guard is cleared by the very
operation it guards.
...
three files (Definitions, Statistics, Cost). Both errors are recorded here rather than corrected
away.
...
2. **One warn-only rule for the whole axis, at the maintainer's direction — RM032.** The
   review refused A2 (a rule for the missing top alone) at 0 of 67, and the first draft of this
   ...
   review's objection to A2 stands on the record: this rule ships on the maintainer's
   definition of a well-formed corpus, not on a measured harm. What the rule does NOT do is
   what the review refused: it writes nothing, it guesses no need, and a need nothing
...
6. **The docstring of `_write_sys_placeholder` now says what the code does.** Three reviewers
...
- Nothing from this record lands in the same release as the v7.0.0 split; the review's
