...
The decisions that shape this project, pulled out of `CHANGELOG.md` prose where they were
previously findable only by reading every release note in order. The index below is the count —
a number written out in words here would be one more thing to keep true by hand.
...
| [0046](0046-the-engine-has-a-line-budget.md) | The engine has a line budget, checked when a release is cut | Accepted (metric superseded by 0053) |
...
| [0048](0048-the-engines-floor-is-what-it-must-keep.md) | The engine's floor is what it must keep, measured by capability | Accepted (amended by 0053) |
