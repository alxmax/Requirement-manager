...
export let TODOS = [];
// The repository's declared requirements language (`en` | `ro` |
// `both`), emitted by the engine from requirements/_config.json. It
// sets the viewer's DEFAULT locale; a reader's own choice
// (localStorage) still wins, because a preference beats a default.
...
/* Planning sidecar from requirements/_planning.json (legacy: `targets`
 * key). Lanes, bars, milestone due dates and the release cadence. Null
 * until loaded. */
export let TARGETS = null;
/* ROADMAP.md's horizon plan, as the engine parsed it: one entry per
 * `- [ ]` / `- [x]` item under Now / Next / Later / Not now, with its
 * `req:` and `unpark:`. Empty for a repo with no ROADMAP.md and for
 * any map written before v7.6.0, which is what gates the Horizons
 * mode off — absent is NOT an empty plan, it is no plan file.
...
export let HISTORY = [];
/* The branch git was on when the map was written, when it could say.
 * Absent is not a wrong answer — a detached HEAD or no work tree both
 * read as unknown, and the band keeps its former label.
 * implements: REQ-PLANBRANCH-1011 */
...
/** Adopt one engine `_map.json` export — the single entry point
 *  loadData uses. */
export function adoptMapExport(data) {
  // implements: REQ-VIEWER-969
  if (!data || typeof data !== "object") return;
  if (Array.isArray(data.nodes)) { REQUIREMENTS = data.nodes; derive(); }
  if ("repo" in data) REPO = data.repo || null;
  if ("language" in data) {
    LANGUAGE = (data.language === "ro" || data.language === "both")
      ? data.language : "en";
  }
...
/* ---- coverage --------------------------------------------------------------
   This is a TEST axis and nothing else. `status: draft` is a REVIEW state
   and is shown by the status pill; folding it in here reported a draft with
   twenty tests as "untested" — a whole corpus of 55 read untested until it
   was promoted, and `health` jumped 0 → 96 without a single test being
   written.
   `clauses`/`covered`/`gap` are emitted by the engine (per labelled
   criterion) or not at all: absent means NOT MEASURED, and nothing here may
   substitute a number of its own. The previous fallback took `clauses` from
   the CONTRACT line count and `covered` from the tested-by badge, so a
   requirement with three real tests displayed "0 / 8 clauses covered". */
export function coverageOf(r) {
  ...
  // engine-measured per-criterion data
  if (r.covered != null && r.clauses) {
    return r.covered >= r.clauses
      ? "tested" : (r.covered > 0 ? "partial" : "untested");
  }
  return hasTest ? "tested" : "untested";
}

/* Why a requirement is exempt — three different reasons that used to render
   as one ("test-exempt — skipped by the gate"), including for a `need` that
   carries real tests and no `test_exempt` at all. */
export function exemptReason(r) {
  ...
  if (r.layer === "need") {
    return "satisfied by other requirements — no code of its own";
  }
  if (r.layer === "aggregate") {
    return "covered by its dependencies — no code of its own";
  }
  return "";
}
