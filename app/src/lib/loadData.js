...
/* The engine's `_acc_items`, ported: one display string per criterion of
 * the raw Cases text, a `CASE-N — …` line per labelled block with its
 * indented Given/When/Then folded in, or the text of a `- ` bullet. The
 * map carried both forms until v8.3.0, 305 KB of this repo's _map.json
 * twice over; it now carries `accept` alone and this derives the folded
 * form. Checked identical on every node of three corpora (497) before the
 * engine stopped emitting `acc`.
...
/** engine node ({...used_by, accept}) → app requirement ({...usedBy, acc,
 * gwt}). */
export function adaptNode(n) {
  // An atomic-form requirement has no Cases text to fold, so the engine
  // still emits its one criterion as `acc`; a map written before v8.3.0
  // carries `acc` for all.
  const acc = Array.isArray(n.acc) ? n.acc : foldAccept(n.accept);
  // the engine always emits a string id, but adaptNode is the trust
  // boundary for any external _map.json — guard it so a malformed node
  // can't throw here
  ...
  return {
    ...
    // Open "verify intent" questions, verbatim. Every bullet the engine
    // emits here is a candidate; the placeholder filter lives in
    // lib/tree.js (openQuestions) so the viewer and `collect_findings`
    ...
    acc,
    // Raw acceptance text: the labelled Given/When/Then block as authored,
    // rendered line for line. `acc` carries the SAME criteria folded to
    // one line each (for search and counting), so gating this on `acc`
    // being empty meant that the day the engine learned to parse the
    // block form (v2.29.0) every criterion silently collapsed into a
    // run-on line. Prefer the authored shape whenever it exists.
    // implements: REQ-VIEWER-942
    gwt: typeof n.accept === "string" && n.accept.trim()
      ? n.accept : undefined,
    members: Array.isArray(n.members) ? n.members : [],
    // `depends_on` since v8.3.0; `deps` is the same list under its old
    // name, read from a map an older engine wrote.
    deps: Array.isArray(n.depends_on)
      ? n.depends_on
      : Array.isArray(n.deps) ? n.deps : [],
    ...
    // `# verifies:` tagging. Undefined means not measured —
    // coverageDetail() then renders no fraction rather than inventing one.
    clauses: n.clauses,
    covered: n.covered,
    gap: n.gap,
    ...
    // translatedText(). Absent for nodes with no `reqmap.py translate`
    // cache.
 ...
 *  1. window.__REQMAP_DATA__  — inlined by the engine into the
 ...
 * `roadmap` and `history` shipped in v7.6.0/v7.8.0 reaching the page and
 * never reaching the app: the engine emitted them,
 * `window.__REQMAP_DATA__` carried them, and this function dropped them
 * on the floor, so the Horizons mode and the Shipped band silently never
 * rendered. `adoptMapExport` reads only the keys it knows and validates
 * each one, so a spread is the safe shape and an added key needs no edit
 * here.  implements: REQ-VIEWER-969 */
export async function loadData() {
  ...
  if (inl && Array.isArray(inl.nodes) && inl.nodes.length) {
    ...
    return {
      source: "inline", engineVersion: inl.engine_version || null,
      count: inl.nodes.length,
    };
  }
  // 2. fetched export (only meaningful over http; file:// will throw →
  // fallback)
  try {
    ...
    return {
      source: "engine", engineVersion: json.engine_version || null,
