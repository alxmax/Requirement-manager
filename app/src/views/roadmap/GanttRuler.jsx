...
/* Sticky: the chart scrolls sideways for months, and a lane the reader
   cannot name is a row of bars with no subject. zIndex clears the bars.
   The scroller pads itself 20px and `left: 0` sticks to the PADDING box,
   so scrolled bars slid through that band beside the names; the shadow
   paints the column's background across it and the scroller clips it at
...
/** The lane-name column. The shipped band is named by the branch git is
 *  on, not the word "Shipped": a map opened from a feature branch looked
 *  identical to one opened from main, right up to the moment someone
 *  acted on the wrong plan (REQ-PLANBRANCH-1011). */
...
const PILL = {
  position: "absolute", top: 4, textAlign: "center", fontSize: 9,
  fontWeight: 800,
...
/** Today, the release ticks and — when no lane holds them — the version
 *  pills. */
  ...
  return (
    <div style={{ height: FLAG_H, position: "relative" }}>
      {todayIdx >= 0 && todayIdx < totalDays && (
        <div style={{
          ...PILL, left: todayIdx * PX + PX / 2 - 18, width: 36,
          color: "var(--accent-2)",
...
/* The day of the month under each week, `day/month`. Today is marked in
   the accent colour and a weekend is fainter, so a Friday reads as the
   end of the week.
   implements: REQ-PLANDAYS-1021 */
export function dayStyle(dd, todayIdx) {
  const today = dd.start === todayIdx;
  return {
    ...CELL, width: PX, flexShrink: 0, overflow: "hidden",
    whiteSpace: "nowrap",
    borderRight:
    ...
    color: today ? "var(--accent-2)"
      : dd.weekend ? fade(55) : "var(--fg-faint)",
    background: dd.weekend ? fade(6) : "none",
  };
}

/** The ruler: flags, then months, ISO weeks and days. An ISO week, not a
 *  count from the chart's left edge: the same calendar week must read
 *  the same in every chart. */
export function Ruler({ lay, t }) {
  const weekStyle = (w) => ({
    ...CELL, width: w.span * PX, fontSize: 9, fontWeight: 600,
    color: "var(--fg-faint)",
    overflow: "hidden", whiteSpace: "nowrap",
  });
  return (
    <div style={{
      position: "relative", height: HEAD_H,
      borderBottom: "1px solid var(--border)",
      ...
      <div style={{ ...ROW, height: MONTH_H }}>
        {lay.months.map((m) => (
          <div key={`${m.label}-${m.start}`} style={{
            ...CELL, width: m.span * PX, fontSize: 11, fontWeight: 700,
            color: "var(--fg-muted)",
          }}>{m.label}</div>
        ))}
      </div>
      <div style={{ ...ROW, height: WEEK_H }}>
        {lay.weeks.map((w) => (
          <div key={w.start} style={weekStyle(w)}>
            {w.span >= 4 ? w.label : ""}
          </div>
        ))}
      </div>
      <div style={{ ...ROW, height: DAY_H }}>
        {lay.days.map((dd) => (
          <div key={dd.start} data-day={dd.label}
               style={dayStyle(dd, lay.todayIdx)}>
            {dd.label}
          </div>
        ))}
      </div>
