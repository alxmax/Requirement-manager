...
// then two, Bugs and Features, and Bugs rendered empty. An axis with one populated value
// sorts nothing while still costing a row, so the lane names what the chips are. Every
// open TODO item and every milestoned requirement lands in it, whatever `lane:` says.
...
/* The two densities differ only in numbers a reader can see the effect of. `titleMax` is
 * the one that reclaims width: a chip stops growing with its title, and the full text
 * moves to the tooltip `Bar` already carries. */
...
export function Bar({ variant, id, label, onClick, d }) {
  ...
  const titleStyle = d.titleMax
    ? { maxWidth: d.titleMax, overflow: "hidden", textOverflow: "ellipsis" } : undefined;
  return (
...
function headStyle(d, isCurrent) {
  const base = {
    ...
    letterSpacing: "0.4px", padding: d.headPad, textAlign: "center", whiteSpace: "nowrap",
    borderBottom: "1px solid var(--border)", borderRight: "1px solid var(--border)",
  };
  if (!isCurrent) return base;
  return { ...base, background: "var(--surface-hov)", color: "var(--fg)", fontWeight: 700 };
}
...
const LABEL_STYLE = {
  fontSize: 9, fontWeight: 400, color: "var(--fg-faint)", marginTop: 2, maxWidth: 140,
...
function VersionHead({ ms, data, d, t, locale }) {
  ...
  return (
    <th style={headStyle(d, isCurrent)}>
      ...
      {meta.due && (
        <div style={{ fontSize: 9, fontWeight: 500, color: dueColor(meta.due), marginTop: 3 }}>
          {t("due {date}", { date: formatDue(meta.due, locale) })}
        </div>
      )}
      {meta.label && <div style={LABEL_STYLE} title={meta.label}>{meta.label}</div>}
    </th>
  );
}
...
function ItemChip({ item, d, openSpec }) {
  if (item?.type === "req") {
    return <Bar variant={barVariant(item.r.status)} id={item.r.id} label={item.r.title}
                onClick={() => openSpec(item.r.id)} d={d} />;
  }
  const idle = () => {};
  if (item?.type === "todo") return <Bar variant="todo" label={item.t.name} onClick={idle} d={d} />;
  if (item?.type === "plan") {
...
function laneCellStyle(d) {
  return {
    ...
    borderRight: "1px solid var(--border)", whiteSpace: "nowrap", background: "var(--bg-raised)",
    minWidth: 60,
  };
}
...
function Row({ rowIdx, data, d, openSpec }) {
  ...
  return (
    <tr>
      {rowIdx === 0 && <td rowSpan={data.maxRows} style={laneCellStyle(d)}>{LANE_LABEL}</td>}
...
function Unscheduled({ data, d, openSpec }) {
  ...
  const toggle = { background: "none", border: "none", cursor: "pointer", padding: "4px 0",
...
/** One column per version. CSS `zoom` (not `transform: scale`) so the scroll extent shrinks
 *  with the content — a transform leaves the container at full size and the reader pans
...
export function VersionsTable({ data, view }) {
  ...
  const corner = { ...headStyle(d, false), background: "transparent", border: "none", width: 60 };
  return (
      ...
      <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "max-content",
        ...
        <thead>
          <tr>
            <th style={corner} />
            {data.milestones.map((ms) => (
              <VersionHead key={ms} ms={ms} data={data} d={d} t={t} locale={locale} />
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((i) => <Row key={i} rowIdx={i} data={data} d={d} openSpec={openSpec} />)}
        </tbody>
      </table>
