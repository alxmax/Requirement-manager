...
/* What the Versions view shows, computed from the export: one column
 * per version and the items in each. Nothing here renders. */
...
/* A milestone whose every TODO item shipped (and no requirement cites
   it) used to have no signal at all reaching this Set — the column
   vanished and the chart jumped straight to the next one, reading as a
   skipped version rather than a finished one. A milestone is real once
   TODO.md groups anything under it, done or not. */
function versionNames(requirements, todos, msMeta, planBars) {
  const names = new Set();
  requirements.forEach((r) => {
    if (r.milestone && live(r)) names.add(r.milestone);
  });
...
/* Planned work is `bars`, the same list the Plan chart draws
   (REQ-PLANSTALE-1013's "planned set"). A bar whose `req:` or title is
   already in the column is not listed twice. */
function addPlannedBars(byMs, planBars) {
  planBars.forEach((bar) => {
    ...
    const dup = col.some((item) =>
      (item.type === "req"
        && (item.r.id === bar.req || item.r.title.toLowerCase() === title))
      || (item.type === "todo" && item.t.name.toLowerCase() === title));
    if (!dup) col.push({ type: "plan", text: bar.title });
  });
}

/** The Versions view's data: columns, their items, the current and
 *  next version, and the planning-level requirements that carry no
 *  milestone. */
export function buildVersions(requirements, todos, targets) {
  ...
  requirements.filter((r) => r.milestone && live(r))
    .forEach((r) => {
      if (byMs[r.milestone]) byMs[r.milestone].push({ type: "req", r });
    });
  todos.filter((t) => !t.done)
    .forEach((t) => {
      if (byMs[t.milestone]) byMs[t.milestone].push({ type: "todo", t });
    });
  ...
  return {
    ...
    /* Unscheduled is a planning bucket, so it holds only what is
       planned AT the planning levels: a `level: code` requirement
       inherits its parent's milestone. */
    unscheduled:
      requirements.filter((r) => !r.milestone && live(r) && r.level !== "code"),
    plannedCount:
      Object.values(byMs).flat().filter((i) => i.type === "plan").length,
