...
/* Calendar Gantt: months, ISO weeks and days on X, swimlanes on Y. Selecting a bar, a
 * version or a shipped month opens its note below (REQ-VIEWER-999). The positions come
 * from `ganttLayout.js`; this file only holds the selection and puts the parts together. */
...
export function PlanGantt(props) {
  const { planning, history, roadmap, branch, locale, t, zoom, openSpec } = props;
  ...
  /* No `overflow: hidden` on the bordered box: it made the box the sticky column's
     scrollport, and a scrollport that never scrolls never lets its sticky child stick.
     `flex: 1` lets the track take the room the lane column leaves; `minWidth: chartW` keeps
     a longer plan at its true scale and scrolls (REQ-PLANSTACK-1012). The note is sticky
     for the same reason as the column: it belongs to the reader, not to the month. */
  return (
    <div style={{ zoom: zoom / 100, width: "max-content", minWidth: "100%" }}>
      ...
      {picked && (
        <div style={{ position: "sticky", left: 0, width: "min(760px, 100%)" }}>
          <PickedNote picked={picked} lay={lay} roadmap={roadmap} t={t} openSpec={openSpec}
                      onPick={setPicked} />
        </div>
      )}
    </div>
