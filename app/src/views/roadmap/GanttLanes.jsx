...
/* The Plan's moving parts: the shipped band, the lanes with their bars and versions, and
 * the guides that tie each of them to its day on the ruler. */
import {
...
function shippedStyle(h, picked) {
  return {
    ...
    width: Math.max((h.endIdx - h.startIdx + 1) * PX - 6, 46), height: ROW_H - 4,
    ...
    cursor: "pointer", outline: selected(picked, `month-${h.month}`), outlineOffset: 1,
  };
}

/** What already shipped, one bar per month; selecting one opens every release in it.
...
export function ShippedBand({ sel, t }) {
  ...
  return (
    <div style={{ position: "relative", height: BAND_H, borderBottom: "1px solid var(--border)",
      background: fade(5) }}>
      <MonthRules months={lay.months} />
      {lay.pastRows.map((h) => {
        const pick = () => toggle({ kind: "month", key: `month-${h.month}`, ...h });
        const span = `${h.versions[0]} → ${h.versions[h.versions.length - 1]}`;
        return (
          <div key={h.month} data-month={h.month} role="button" tabIndex={0}
            onClick={pick} onKeyDown={onActivate(pick)} style={shippedStyle(h, picked)}
...
/** A version is a release: its pill sits in the cadence's lane at its due date, and
 *  selecting it opens what is planned on it.  implements: REQ-PLANCADENCE-1000 */
function VersionPill({ f, laneH, sel }) {
  const { lay, picked, toggle } = sel;
  const key = `ms-${f.ms}`;
  const pick = () => toggle({ kind: "version", key, ...f });
  return (
    <div data-version={f.ms} role="button" tabIndex={0} onClick={pick} onKeyDown={onActivate(pick)}
      title={`${f.ms} · ${f.label ? `${f.label} · ` : ""}${f.at.toLocaleDateString(lay.loc)}`}
      style={{
        cursor: "pointer", outlineOffset: 1, outline: selected(picked, key),
        position: "absolute", top: laneH / 2 - 11, left: f.idx * PX + PX / 2 - 34,
...
function barStyle(bar, tone, picked) {
  ...
  return {
    position: "absolute", left, top: PAD + bar.subRow * ROW_H, width, height: ROW_H - 6,
    background: tone.bg, color: tone.fg, borderRadius: 4, boxSizing: "border-box",
    border: `1px solid color-mix(in oklch, ${tone.edge} 40%, transparent)`,
    borderLeft: `3px solid ${tone.edge}`, fontSize: 11, fontWeight: 600, padding: "4px 8px",
    display: "flex", alignItems: "flex-start", overflow: "hidden", cursor: "pointer",
    outline: selected(picked, bar.key), outlineOffset: 1,
  };
}

/* Three lines, then an ellipsis. A title cut mid-word on one line told the reader nothing
   about how much it was missing. */
const TITLE = {
...
const PROGRESS = {
  position: "absolute", inset: 0, background: "color-mix(in oklch, var(--fg) 8%, transparent)",
  pointerEvents: "none",
};
...
function Bar({ bar, tone, picked, toggle }) {
  ...
  return (
    <div title={`${bar.title}\n${bar.start} → ${bar.end}${milestone}`}
      ...
      {bar.progress != null && <span style={{ ...PROGRESS, width: `${bar.progress}%` }} />}
      <span style={TITLE}>{bar.title}</span>
    </div>
  );
}

/** One lane: month rules, the version pills when it is the release lane, and its bars. */
export function Lane({ ln, i, sel }) {
  ...
  return (
    <div style={{ position: "relative", height: laneH,
      borderBottom: i < lay.lanes.length - 1 ? "1px solid var(--border-soft)" : "none" }}>
      <MonthRules months={lay.months} />
      {pills.map((f) => (
        <VersionPill key={`ms-${f.ms}`} f={f} laneH={laneH} sel={sel} />
      ))}
      {(lay.byLane[ln] || []).map((bar) => (
        <Bar key={bar.key} bar={bar} tone={tone} picked={picked} toggle={toggle} />
      ))}
    </div>
  );
}
...
/** Guides run from the day on the ruler down to the thing that happens on it, and stop
 *  there: a bar's start (solid, a commitment) and end (dotted, an estimate), and a version
 *  at its due day.  implements: REQ-PLANSTACK-1012 */
export function Guides({ guides }) {
  return guides.map((gd) => (
    <div key={gd.key} data-guide={gd.kind} style={{
      ...
      borderLeft: GUIDE_BORDER[gd.kind], opacity: gd.kind === "end" ? 0.38 : 0.5,
