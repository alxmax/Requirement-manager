...
/* Where everything on the Plan chart goes, computed once from the export and handed to
 * the components that draw it. Nothing here renders; nothing in the components measures. */
import {
  parseIso, isoLocal, dayIndex, buildMonthBands, buildWeekBands, buildDayBands, stackBars,
...
export const LANE_TONE = [
  { bg: tint("var(--cov-tested)", 22), fg: "var(--cov-tested)", edge: "var(--cov-tested)" },
  { bg: tint("var(--amber-600)", 20), fg: "var(--amber-700)", edge: "var(--amber-600)" },
  { bg: "var(--indigo-tint)", fg: "var(--indigo-500)", edge: "var(--indigo-400)" },
  { bg: tint("var(--fg-faint)", 14), fg: "var(--fg-muted)", edge: "var(--fg-faint)" },
];
...
/** The chart's first and last day: whole months around every dated thing and today.
 *  Release dates extend the range like any other dated thing: a cadence running past the
 *  last bar — `until: 2026-12-31` with nothing scheduled in December — otherwise emits
 *  dates the chart drops, and the engine says a release lands where the chart shows none. */
function chartRange(raw, dueList, past, releases, todayD) {
  ...
  const origin = monthStart(new Date(Math.min(...dates.map((d) => d.getTime()))));
 ...
 * `stackBars` call that sized the lanes. A guide runs from its day to its bar or version
 * and stops there.  implements: REQ-PLANSTACK-1012 */
function buildGuides(lay) {
  const laneTop = {};
  lay.lanes.reduce((top, ln, i) => { laneTop[ln] = top; return top + lay.heights[i]; },
                   lay.pastRows.length > 0 ? BAND_H : 0);
  const barGuides = lay.bars.filter((b) => laneTop[b.lane] != null).flatMap((b) => {
...
/** Everything the chart draws, positioned; `null` when the plan holds nothing dated.
 *  A cadence with no bars is still a calendar, and a repo that has planned nothing is the
 *  one that needs one — `init` seeds it for exactly that (REQ-PLANHORIZON-1010). */
export function layoutPlan(planning, history, locale) {
   ...
   * engine, on the same timeline as the plan.  implements: REQ-HISTORY-1003 */
  const past = (history || []).filter((h) => parseIso(h.first) && parseIso(h.last));
  if (!raw.length && !dueList.length && !past.length && !releases.length) return null;

  const { origin, totalDays } = chartRange(raw, dueList, past, releases, todayD);
  ...
  const lay = {
    ...
    heights: lanes.map((ln) => Math.max(stackBars(byLane[ln] || [], extent), 1) * ROW_H + PAD * 2),
    pastRows: past.map((h) => {
      const startIdx = dayIndex(origin, parseIso(h.first));
      return { ...h, startIdx, endIdx: Math.max(dayIndex(origin, parseIso(h.last)), startIdx) };
    }),
     ...
     * places them, so the CLI and the chart cannot disagree about when a release lands.
     * implements: REQ-PLANCADENCE-1000 */
    releaseIdx: releases.map((iso) => ({ iso, idx: dayIndex(origin, parseIso(iso)) }))
      .filter((r) => r.idx >= 0 && r.idx < totalDays),
    /* A version is a release, so it is drawn in the lane the cadence names; with no such
     * lane the pill stays on the ruler, where a plan without a cadence has always shown it. */
    releaseLane: lanes.includes(planning?.cadence?.lane) ? planning.cadence.lane : null,
