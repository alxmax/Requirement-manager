...
/* The note that opens below the Plan when a bar, a version or a shipped month is selected. */
...
const REQ_LINK = { ...LINK, color: "var(--accent-2)", textDecoration: "underline" };
const CLOSE = {
...
export function ShippedNote({ month, t, onClose }) {
  ...
  const meta = `${month.count} ${say(t, "releases")} · ${month.first} → ${month.last}`;
  return (
...
function PlannedBar({ bar, openSpec, onPickBar }) {
  return (
    <li style={NOTE_ROW}>
      ...
      {bar.reqId && openSpec && (
        <button type="button" onClick={() => openSpec(bar.reqId)} style={REQ_LINK}>
          {bar.reqId}
        </button>
      )}
    </li>
...
export function VersionNote({ version, bars, t, openSpec, onClose, onPickBar }) {
  ...
  const list = (
    <ul style={NOTE_LIST}>
      {planned.map((b) => (
        <PlannedBar key={b.key} bar={b} openSpec={openSpec} onPickBar={onPickBar} />
      ))}
    </ul>
  );
  return (
    <div style={NOTE_BOX} data-note="version">
      ...
      {planned.length
        ? list
        : <div style={FAINT}>{say(t, "Nothing planned on this version yet.")}</div>}
    </div>
  );
}

/** A bar, opened: its dates, its requirement and the note under its ROADMAP.md item.
...
function BarNote({ bar, roadmap, t, openSpec, onClose }) {
  ...
  const req = bar?.reqId || bar?.req;
  const meta = `${bar.start} → ${bar.end}${bar.milestone ? ` · ${bar.milestone}` : ""}`
    + `${item?.horizon ? ` · ${item.horizon}` : ""}`;
  const reqLine = openSpec
    ? <button type="button" onClick={() => openSpec(req)} style={REQ_LINK}>{req}</button>
    : <span style={{ color: "var(--fg-faint)" }}>{req}</span>;
  const noteStyle = { marginTop: 10, fontSize: 12.5, lineHeight: 1.55, whiteSpace: "pre-wrap" };
  return (
    <div style={NOTE_BOX}>
      ...
      {note
        ...
        : <div style={FAINT}>{say(t, "No note in ROADMAP.md for this item.")}</div>}
    </div>
  );
}
...
export function PickedNote({ picked, lay, roadmap, t, openSpec, onPick }) {
  ...
  if (picked.kind === "month") return <ShippedNote month={picked} t={t} onClose={close} />;
  if (picked.kind === "version") {
    return <VersionNote version={picked} bars={lay.bars} t={t} openSpec={openSpec}
                        onClose={close} onPickBar={onPick} />;
  }
  return <BarNote bar={picked} roadmap={roadmap} t={t} openSpec={openSpec} onClose={close} />;
}
