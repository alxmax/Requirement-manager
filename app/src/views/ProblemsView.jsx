...
const SEV = {
  unimplemented: {
    sev: "ERROR", msg: "No implementing member — orphan. The gate blocks the build.",
  ...
  "unverified-intent": {
    ...
    fix: "Answer each one (`sync` lists them in _findings.md), then fold the answer into the "
      + "Description or delete the bullet.",
  },
  unreviewed: {
    sev: "REVIEW", msg: "Drafted from code by extract — intent not yet validated.",
  ...
  REQUIREMENTS.forEach((r) => {
    ...
    if (cov === "partial") {
      out.push({ id: r.id, title: r.title, signal: "partial", sev: "WARN",
        msg: `Partial coverage — ${r.covered}/${r.clauses} criteria verified. ${r.gap || ""}`
          .trim(),
      ...
      out.push({ id: r.id, title: r.title, signal: "untested", sev: "WARN",
        msg: "Untested — no live acceptance test, or a `tested-by` range that no longer "
          + "resolves (stale ref).",
...
export function ProblemsView({ openSpec, problems }) {
  ...
  const gateMsg = (counts.ERROR || 0) > 0
    ? <>
        <Icon name="triangle-alert" size={14} style={{ color: "var(--status-error)" }} />
        {" "}{t("gate blocks the build — {n} error", { n: counts.ERROR })}
      </>
    : <>
        <Icon name="shield-check" size={14} style={{ color: "var(--status-confirmed)" }} />
        {" "}{t("gate passes")}
      </>;
