...
import { ProblemTabBar, DesignProblemsPanel, ProblemRow, ProblemsEmpty } from "./problems/ProblemsPanels.jsx";
...
export function ProblemsView({ openSpec, problems }) {
  ...
  const counts = all.reduce((a, p) => { a[p.sev] = (a[p.sev] || 0) + 1; return a; }, { all: all.length });
  const design = (DESIGN && Array.isArray(DESIGN.findings)) ? DESIGN.findings : [];
  const advice = (DESIGN && DESIGN.advice) || {};
  ...
  return (
    <div className="main">
      <ProblemTabBar filter={filter} setFilter={setFilter} counts={counts} designCount={design.length} gateMsg={gateMsg} />
      <div className="problems">
        ...
        {filter !== "DESIGN" && (hidden > 0 || (showDrafts && draftReviews > 0)) && (
          <button type="button" className="prob-chip" onClick={() => setShowDrafts((s) => !s)}>
            {showDrafts ? t("hide {n} draft review rows", { n: draftReviews }) : t("{n} draft review rows hidden — show", { n: hidden })}
          </button>
        )}
        {filter !== "DESIGN" && shown.map((p, i) => <ProblemRow key={i} p={p} openSpec={openSpec} t={t} />)}
