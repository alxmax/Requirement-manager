...
/** One zoomable map. `sel` is the selection every map shares: `selId`, `setSelId`,
 *  `highlightId`, `edgeEnds`, `selKey` and `setSelEdge`. */
function ZoomedCanvas({ meta, label, markerId, extraNodes, sel }) {
  ...
  return (
    <div className="map-main">
      ...
      <MapCanvas size={meta} zoom={zoom} pan={pan} onClear={() => sel.setSelEdge(null)}>
        {label && <div className="subgraph-label" style={{ left: 50, top: 12 }}>{label}</div>}
        <MapEdges meta={meta} selKey={sel.selKey} onSelect={sel.setSelEdge} markerId={markerId} />
        {REQUIREMENTS.filter((r) => meta.pos[r.id]).map((r) => (
          <MapNodeBox key={r.id} r={r} pos={meta.pos} marks={marksFor(r.id, sel)}
...
/** Coupling between areas: one pseudo-node per id area, an edge where one depends on another. */
function areaGraph(nodes) {
  ...
  const pseudo = Object.keys(byArea).map((a) => ({
    id: a,
    deps: [...pairs].filter((e) => e.startsWith(a + "\u0001")).map((e) => e.split("\u0001")[1]),
  }));
  return { layout: computeLayout(pseudo, { colW: 320, rowH: 150 }), count: byArea };
}

/** The graphs the map tabs draw, laid out once. The code level is read in the Explorer. */
function useMapGraphs() {
  ...
  const risk = useMemo(() => computeLayout(flagged, { colW: 300, rowH: 150 }), [flagged]);
...
const LEGEND = {
  system: <>
    <span className="pdot" style={BUS_SWATCH} /> bus · arrows = depends_on · edge colour = source
  </>,
  reqcode: <>
    requirement → its code · <span style={{ color: "var(--accent)" }}>implements</span> / tested-by
  </>,
...
function riskClassOf(r) {
  ...
  return sig === "drift" || sig === "blast-radius" || sig === "unreviewed" ? "risk-blast" : "";
}

function riskNodes(g, sel) {
  return g.flagged.filter((r) => g.risk.pos[r.id]).map((r) => (
    <MapNodeBox key={r.id} r={r} pos={g.risk.pos} marks={marksFor(r.id, sel, riskClassOf(r))}
                onClick={sel.setSelId} />
  ));
}
...
function MapBody({ tab, g, sel }) {
  if (tab === "system") {
    return <ZoomedCanvas meta={g.sys} markerId="arrow-sys" sel={sel}
      label="consumers → depends on → foundation · drag to pan · click a line to trace it" />;
  }
  if (tab === "reqcode") {
    return <ReqCodeView selId={sel.selId} setSelId={sel.setSelId} rows={g.nodes} />;
  }
  if (tab === "deps") {
...
export function MapView({ selId, setSelId, openSpec, highlightId, setHighlightId }) {
  ...
  return (
    <div className="main">
      <MapTabs tab={tab} pick={pick} hiddenCode={REQUIREMENTS.length - g.nodes.length} t={t} />
      <div className={"map-wrap" + (current ? " with-panel" : "")}>
        ...
        {current && <MapDetailPanel r={current} onClose={() => setSelId(null)}
          onLocate={() => locateNode(selId, setHighlightId)} onOpenSpec={openSpec} />}
      </div>
    </div>
  );
}
