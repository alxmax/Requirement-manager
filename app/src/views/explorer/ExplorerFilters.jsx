...
/** The filter chips above the tree. `filters` is the state
 *  `useExplorerFilters` returns; `tree` carries the tally and the
  ...
  return (
    <div className="ex-filters">
      <div className="ex-filter-row">
        ...
        {LEVELS.map((l) => (
          <Chip key={l} on={!!f.level[l]} onClick={() => f.flipLevel(l)}>
            {LEVEL_LABEL[l]}
          </Chip>
        ))}
      </div>
      <div className="ex-filter-row">
        ...
        {STATUSES.map((s) => (
          <Chip
            key={s}
            on={!!f.status[s]}
            onClick={unfocus(() => f.flipStatus(s))}
          >
            {s}
          </Chip>
        ))}
        <Chip
          ...
          onClick={unfocus(() => f.setOnlyOrphans((v) => !v))}
        >
          orphan
        </Chip>
      </div>
      ...
      <div className="ex-tally">
        ...
        {tree.flat && (
          <span className="ex-flatnote">
            {t("· no hierarchy in this map — flat list")}
          </span>
        )}
      </div>
    </div>
