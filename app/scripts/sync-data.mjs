 ...
 *
 * `--require` makes that fallback fatal. The SSR smoke exists to check the viewer
 * against this repo's REAL registry; with the export missing it would silently run
 * against the baked stand-in and report green, so CI passes the flag and a missing
 * export fails loudly there while a local `npm run sync` still falls back. */
...
if (!src) {
  ...
  if (required) {
    console.error("[sync] --require: refusing to fall back to the baked dataset.");
    process.exit(1);
  }
