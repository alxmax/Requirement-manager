...
// tested-by: REQ-VIEWER-942
// tested-by: REQ-VIEWER-943
// tested-by: ARCH-VIEWER-007
// tested-by: REQ-VIEWER-944
// tested-by: REQ-VIEWER-945
// tested-by: REQ-VIEWER-966
// tested-by: ARCH-SEARCH-036
// tested-by: REQ-SEARCH-965
// tested-by: REQ-VIEWER-969
...
for (const [name, el] of Object.entries(cases)) {
  try {
    ...
    if (html.length < 200) {
      fail(`FAIL ${name}: output too short (${html.length})`);
...
// tested-by: REQ-TRANSLATE-996
// SSR has no window; a minimal one with only the inlined blob is exactly
// the state the provider sees at mount time in the single-file viewer (no
// localStorage = no reader choice).
{
  ...
  const roHtml = renderWith("ro"), enHtml = renderWith("en");
  const bothHtml = renderWith("both");
...
// ranked-search parity (ARCH-SEARCH-036): the viewer's search must rank by
// the SAME TF-IDF model as the engine `search` CLI. This golden fixture is
// asserted identically in the Python `Search` tests (class SearchParity) —
// the query scores 0.4112 on REQ-DRIFT-001 in BOTH runtimes, and a
// no-overlap query floors out to nothing. If either runtime drifts from
// the model, one of these fails.
...
const ranked = rankRequirements(
  SEARCH_FIXTURE, "contract changed against the lock hash");
const nomatch = rankRequirements(
  SEARCH_FIXTURE, "banana photosynthesis wombat");
const searchChecks = [  // tested-by: REQ-SEARCH-912
  ["ranked search returns the drift requirement first",
    ranked[0]?.req.id === "REQ-DRIFT-001"],
...
adoptMapExport({ nodes: [
  adaptNode({ id: "LINK-SRC-001", title: "source", area: "LINK",
    layer: "feature",
    status: "confirmed",
    intent: "i",
    contract: ['see [[LINK-DST-002]] and [[LINK-GONE-999]] <b>x</b>'],
    acc: [], members: [], deps: [], used_by: [] }),
  adaptNode({ id: "LINK-DST-002", title: "target", area: "LINK",
    layer: "feature",
    status: "confirmed",
    intent: "i", contract: ["a clause"], acc: [], members: [], deps: [],
    used_by: [] }),
] });
...
const linkChecks = [
  ...
  ["links: a resolvable cross-reference renders as a control carrying the id",
    linkSpec.includes('data-req="LINK-DST-002"')
      && !linkSpec.includes("[[LINK-DST-002]]")],
  // verifies: REQ-VIEWER-944#CASE-2
  ["links: a dangling cross-reference is marked, not linked",
    linkSpec.includes("wikilink off")
      && !linkSpec.includes('data-req="LINK-GONE-999"')],
  // verifies: REQ-VIEWER-944#CASE-3
  ["links: markup beside a cross-reference stays escaped",
...
// provider's own initial value comes from localStorage, which does not
// exist here.
adoptMapExport({ nodes: json.nodes.map(adaptNode) });
...
const i18nChecks = [
  ...
  ["i18n: English is the default rendering",
    specEn.includes("Where — Members in code")],
  // verifies: REQ-VIEWER-943#CASE-2
  ["i18n: Romanian translates a section header",
    specRo.includes("Unde — Membri în cod")
      && !specRo.includes("Where — Members in code")],
...
// i18n content translation (opt-in, cached, always marked) —
// REQ-TRANSLATE-042. A node with a cached en-locale translation renders the
// translated text WITH the "machine-translated, unreviewed" badge; a node
// with no cache entry (the default for every requirement until
// `reqmap.py translate` runs) renders the author's text and shows no badge
// at all — the untranslated path must stay unchanged.
adoptMapExport({ nodes: [adaptNode({
  id: "I18N-CONTENT-TEST-001", title: "Titlu original", area: "I18N",
  layer: "feature",
  status: "confirmed", intent: "Motivul original.",
  contract: ["- Clauza originală."],
  acc: ["- Criteriul original."], members: [], deps: [], used_by: [],
  i18n: { en: { title: "Original title", intent: "The original reason.",
                contract: "- The original clause.",
                acceptance: "- The original criterion." } },
})] });
const translatedSpecEn = renderToString(
  <I18nProvider initialLocale="en">
    {specOf("I18N-CONTENT-TEST-001")}
  </I18nProvider>);
...
adoptMapExport({ nodes: [adaptNode({
  id: "I18N-NOCACHE-TEST-001", title: "Titlu fără cache", area: "I18N",
  layer: "feature",
  status: "confirmed", intent: "Motiv.", contract: ["- Clauză."],
  acc: ["- Criteriu."],
  members: [], deps: [], used_by: [],
})] });
const noCacheSpecEn = renderToString(
  <I18nProvider initialLocale="en">
    {specOf("I18N-NOCACHE-TEST-001")}
  </I18nProvider>);
...
const i18nContentChecks = [
  ...
  ["i18n content: no cache entry for ro falls back to the author's title",
    translatedSpecRo.includes("Titlu original")
      && !translatedSpecRo.includes("Original title")],
  // verifies: REQ-TRANSLATE-938#CASE-4
  ...
  // COUNT, not presence. The four checks above assert the badge string
  // appears SOMEWHERE in the rendered document, which no single render
  // site owns: a mutation matrix over all four `<TranslatedBadge />` sites
  // in SpecDoc.jsx (title, contract, intent, acceptance) killed 0 of 4 —
  // deleting any one left every check green, so a refactor could drop a
  // badge and ship machine-translated prose as the author's own with
  // nothing failing. The fixture caches all four fields, so a
  // fully-translated node owes exactly four badges.
  // verifies: REQ-TRANSLATE-938#CASE-4
  ["i18n content: every translated field carries its own badge, not just "
    + "one somewhere",
    (translatedSpecEn.match(/machine-translated, unreviewed/g) || [])
      .length === 4],
  // The boundary the feature exists to respect: the CHROME toggle never
  // translates the artifact under review. Asserted on a requirement with
  // NO `i18n` cache entry, because a cached translation IS rendered, with
  // a badge — that is REQ-TRANSLATE-938's job and a different axis. Until
  // the Spec tab was removed this read `specRo.includes(title)` against
  // ARCH-MAP-007, which the repo's own cache DOES translate; it passed
  // only because the tab's 220px nav listed every title untranslated
  // beside the document.
  // verifies: REQ-VIEWER-943#CASE-5
  ["i18n: the chrome toggle leaves an untranslated requirement alone",
    noCacheSpecRo.includes("Titlu f")
      && !noCacheSpecRo.includes("machine-translated, unreviewed")],
];
for (const [label, ok] of i18nContentChecks) test(label, ok);
// ---- acceptance criteria keep their Given/When/Then lines ------------------
// The engine emits BOTH `accept` (the raw labelled Gherkin block) and `acc`
// (the same criteria folded to one line each, for search and counting).
// `gwt` used to be set only when `acc` was empty — true for every
// requirement until the engine learned to parse the block form (v2.29.0),
// and false for every one after, which silently turned every criterion
// into a single run-on line.
const GWT_ACCEPT = "AC-1\n  Given  a repo with no requirements/\n"
  + "  When   `init` runs\n"
  + "  Then   it creates the directory";
adoptMapExport({ nodes: [adaptNode({
  id: "GWT-TEST-001", title: "Acceptance block", area: "GWT",
  layer: "feature",
...
const gwtChecks = [
  ...
  ["acceptance: a labelled block renders as the multi-line gwt block, not "
    + "a folded bullet",
  ...
  ["acceptance: adaptNode still exposes acc for search and counting",
    adaptNode({ id: "X", acc: ["AC-1 — a"], accept: GWT_ACCEPT })
      .acc.length === 1],
];
...
// viewer still has to draw the registry. Longest-path ranking never
// converges on a cycle: it ran its full pass budget and returned maxRank
// 236 for a real 59-node corpus (a DAG of 59 cannot exceed 58), i.e. a
// 71,000px canvas of empty columns.
const cyc = [
  { id: "A-1", deps: ["B-2"] },
  { id: "B-2", deps: ["C-3"] },
...
const layoutChecks = [
  ...
  ["layout: a cycle cannot rank beyond the node count",
  ...
  ["layout: every node still gets a position",
...
const folded = adaptNode({ id: "FOLD-001", title: "t", accept: FOLD_ACCEPT,
                           depends_on: ["DEP-001"] });
const legacy = adaptNode({
  id: "FOLD-002", title: "t", acc: ["AC-1 — kept"], deps: ["OLD-001"],
});
const foldChecks = [
  ...
  ["fold: a node with no acc gets one folded entry per criterion",
    JSON.stringify(folded.acc) === JSON.stringify(
      ["CASE-1 — — first Given  a When   b Then   c",
        "CASE-2 — Given  d  Then   e"])],
