...
/* What every part of the render smoke test shares: the real engine
 * export, adopted once, and the `test(label, ok)` helper that counts
 * failures. */
...
export const noop = () => {};
// The Spec TAB was removed (Explorer renders the same document beside
// a richer nav); every check below was about the DOCUMENT, so each one
// renders it directly.
// REQ_BY_ID is a live binding, so this reads whatever fixture was
// last adopted.
...
/** A failure that is not a labelled check, such as a view that threw
 *  while rendering. */
