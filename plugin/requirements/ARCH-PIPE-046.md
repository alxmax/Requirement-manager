...
## Description
> `reqmap.py ask --dupes | head` is the natural way to look at a long report. On Windows there
...
**Example**
`python -X utf8 scripts/reqmap.py ask --dupes | head -12` on a 1,141-requirement corpus prints
...
## Description
> `reqmap.py ask --dupes | head` closes the pipe as soon as `head` has enough. On Windows that
