...
- `map` copies `_map.html` to `docs/map.html` whenever a `docs/` directory at the git root carries a GitHub Pages signal (`.nojekyll` or `index.html`), and `map --check` then guards that published copy against drifting from a fresh render. [[REQ-PAGES-889]]
...
# Publish docs/map.html and flag it when it drifts

## Description
> Anyone can open a URL; almost nobody clones a repo just to double-click a file. Copying
> `_map.html` into `docs/map.html` on every `map` run turns the viewer into a page GitHub
> Pages serves directly. Without the paired freshness check in `map --check`, that published
> copy could be hand-edited or simply forgotten and would silently drift from the registry
> it claims to show.
...
## Cases
CASE-1 — map copies the viewer to docs/map.html only when a Pages signal exists
...
CASE-2 — check flags an edited docs/map.html as stale, absent copy as fresh
...
CASE-3 — a swapped repo field alone does not mark docs/map.html stale
