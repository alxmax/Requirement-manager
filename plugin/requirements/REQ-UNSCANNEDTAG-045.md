...
- Tracked, not "every file on disk": the walk that discovers members is bounded by the
  ...
  fast). One `git ls-files` call bounds this check the same way `REQ-TRACKED-042` is
...
## Links
- Sibling: [[REQ-TRACKED-042]] (the same `git ls-files` boundary, other direction).
