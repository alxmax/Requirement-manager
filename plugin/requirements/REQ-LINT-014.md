...
- `cmd_lint`, `lint_requirement`, `_lint_prose`, `_sentences` and `_clip` in `reqmap.py` — `cmd_lint` selects non-draft requirements, runs `lint_requirement` on each, prints findings grouped per requirement, and decides the exit code. `_lint_prose` extracts the prose lines of one section; `_sentences` splits a line for the word-count check. The `missing-section` check reuses `_has_section` (shared with the gate in `cmd_check`).
