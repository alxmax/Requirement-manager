...
# load_requirements returns one meta/body/path record per file

## Description
> Every other command — gate, map, next — works from an in-memory dict of requirement
> records, not the raw Markdown. `load_requirements` builds that dict once: one entry per
> file, keyed by id, holding the parsed frontmatter (`meta`), the prose after it (`body`),
...
## Cases
CASE-1 — load_requirements returns a meta/body/path record per file
...
CASE-2 — parse_frontmatter splits the header fields from the markdown that follows
  Given  `"---\nid: REQ-A-001\nstatus: draft\n---\nbody text\n"`
  When   `parse_frontmatter(text)` runs
  Then   `meta["id"] == "REQ-A-001"` and `body == "body text\n"`, with no `---` markers in either

CASE-3 — a file with no id: field is keyed by its filename stem
...
# The hand-rolled frontmatter grammar: scalars and lists only

## Description
> `parse_frontmatter` reads the `---`-delimited header without a YAML library, so the
> engine stays stdlib-only. It supports what requirement files actually use — scalars,
> inline `[a, b]` lists, block-style lists — and nothing more: no nested mappings, no
> multi-line scalars. Comments and quotes are stripped so a value is clean either way.
...
## Cases
CASE-1 — a block-style key: / indented - item list parses to a Python list
  Given  `"---\ndepends_on:\n  - A-B-001\n  - C-D-002\n---\n"`
  When   `parse_frontmatter(text)` runs
  Then   `meta["depends_on"] == ["A-B-001", "C-D-002"]`

CASE-2 — a trailing # comment does not leak into the parsed value
  Given  `"---\nstatus: draft  # not enforced\n---\n"`
  When   `parse_frontmatter(text)` runs
  Then   `meta["status"] == "draft"`, with the comment text absent

CASE-3 — matching quote characters are stripped from a scalar value
  Given  `'---\nid: "REQ-X-001"\nstatus: \'draft\'\n---\n'`
  When   `parse_frontmatter(text)` runs
  Then   `meta["id"] == "REQ-X-001"` and `meta["status"] == "draft"`, quotes removed

CASE-4 — an unclosed inline list still parses as a list, not a literal string
  Given  `"---\ntags: [x, y\n---\n"` (no closing `]`)
  When   `parse_frontmatter(text)` runs
...
# Missing frontmatter, underscore files, and a BOM never crash the reader

## Description
> A requirements directory is not always full of well-formed files: `_reqlock.json` and
> other generated `_`-prefixed files sit next to real requirements, some files may be
> plain prose with no frontmatter block, and an editor may have saved one with a leading
> byte-order mark. `load_requirements` treats all three as ordinary inputs, not errors.
...
## Cases
CASE-1 — a file with no frontmatter block yields empty meta and the full text as body
  Given  `"# Title\njust text\n"` (no leading `---`)
  When   `parse_frontmatter(text)` runs
  Then   `meta == {}` and `body == "# Title\njust text\n"`

CASE-2 — an underscore-prefixed file is excluded from load_requirements
...
CASE-3 — a leading UTF-8 BOM does not break frontmatter parsing
