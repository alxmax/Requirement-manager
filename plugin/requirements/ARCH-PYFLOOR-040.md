...
- `MIN_PYTHON` names the oldest interpreter version the engine supports, and `reqmap.py` refuses to run below it with a readable message and exit code 2. [[REQ-PYFLOOR-902]]
...
# Refusing an interpreter below the declared floor

## Description
> A floor nothing tests is a claim, not a guarantee: `MIN_PYTHON` is deliberately pinned to
> the oldest interpreter CI actually runs, not the oldest one the code happens to work on. The
> refusal fires before any command dispatches, so an old interpreter fails with one readable
> line naming the fix instead of a stdlib `AttributeError` buried deep in some command.
...
## Cases
CASE-1 — MIN_PYTHON names a concrete required version
...
CASE-2 — MIN_PYTHON matches the CI matrix's oldest entry
...
CASE-3 — an interpreter below the floor is refused before any command runs
...
CASE-4 — the refusal exits 2 and names both versions and the fix
...
CASE-5 — _python_floor_error is testable without an old interpreter
