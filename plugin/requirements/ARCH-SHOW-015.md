...
- `show <ID>` prints one consolidated, read-only, human-readable view of a single requirement: a header, the full intent, and the contract bullets. [[REQ-SHOW-917]]
- `show` prints dependencies in both directions (the `depends_on` ids and the reverse edges) and the code members grouped by role with their `file:line`. [[REQ-SHOW-918]]
- `show` lists the open `## Verify intent` questions and the risk signals with their advice, reusing the same sources as `findings` and `next`. [[REQ-SHOW-919]]
- `show` returns zero for a known id and a non-zero code with a clear message for an unknown one. [[REQ-SHOW-919]]
...
# A one-screen header, intent and contract

## Description
> Understanding one requirement used to mean opening its file, then hunting through other
> files for what depends on it and what code carries it out. `show <ID>` is read-only and
> puts the header (id, status, layer and optional fields), the full multi-line intent, and
> the contract bullets on one screen — or says plainly that no such requirement exists.
...
## Cases
CASE-1 — one show call surfaces header, contract, deps, and members together
...
CASE-2 — show leaves the requirement file byte-identical on disk
...
CASE-3 — the header line carries id, status, and layer
...
CASE-7 — a requirement with no Description section says so instead of listing bullets
...
# Dependencies both ways, and the code members

## Description
> Knowing what a requirement will affect means seeing both what it depends on and what
> depends on it, plus exactly which code implements it. `show` computes the reverse edges by
> scanning every requirement's `depends_on` on each call — the corpus is small enough that
> this needs no cache — and lists members with their verification level when their
> `tested-by:` tag carries one.
...
# Open questions, risk signals, and a caller-visible exit code

## Description
> A dossier is more useful when it also flags what still needs attention: open verify-intent
> questions and risk signals, both drawn from the same source `findings` and `next` use so
> the views can never disagree. And a typo'd id must be caught rather than silently produce
> an empty-looking report, so `show` exits non-zero with a clear message when the id does
> not exist.
...
## Cases
CASE-1 — an open verify-intent bullet shows; the None placeholder does not
...
CASE-2 — show prints a risk signal alongside its advice text
...
CASE-3 — exit code distinguishes a known id from an unknown one
