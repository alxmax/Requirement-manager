...
|---|---|---|
| **init / reinit everything** | Hard reset: delete all non-generated requirement files (`requirements/*.md` where name does not start with `_`), then re-initialize from scratch. Re-drafts all code/prose into fresh requirements and rebuilds the lock + map. Never clobbers `.reqmapignore`. (PowerShell: `Get-ChildItem requirements\*.md \| Where-Object { $_.Name -notlike '_*' } \| Remove-Item`; POSIX: `find requirements -maxdepth 1 -name '*.md' ! -name '_*' -delete`) | delete non-generated `requirements/*.md` → `python scripts/reqmap.py init` |
| **regenerate requirements** | Hard reset: delete all non-generated requirement files (`requirements/*.md` where name does not start with `_`), then re-extract from scratch. Covers code and prose (`.md`/`.html`). Afterwards, tell the user which drafts were created and remind them to review + `promote` the real ones. (PowerShell: `Get-ChildItem requirements\*.md \| Where-Object { $_.Name -notlike '_*' } \| Remove-Item`; POSIX: `find requirements -maxdepth 1 -name '*.md' ! -name '_*' -delete`) | delete non-generated `requirements/*.md` → `python scripts/reqmap.py extract` → `scan` → `check --update-lock` → `map` |
