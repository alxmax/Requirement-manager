...
- `python scripts/reqmap.py clarify AREA-NAME-NNN` — Ask what a requirement has not answered yet: vague terms with no threshold, numbers with no unit, unbounded quantities, clauses with no case, a missing failure path. Read-only, always exit 0, never a gate rule. --decompose is the write half of the same question: it scaffolds an over-scoped requirement's clauses into requirements of their own. Run it before implementing, so the ambiguity is resolved in the requirement instead of guessed in code. Flags: `--decompose` Scaffold an over-scoped requirement's clauses into requirements of their own.; `--levels` Propose a V-model rung for every requirement that declares no `level:`; --apply writes them, each marked `level_source: auto`.; `--json` Emit the questions as JSON for an agent to answer..
