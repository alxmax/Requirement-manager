...
The plan answers "what is next". This answers "what happened", and it
needs no new file to do it: a repo that follows the release discipline
already writes one dated heading per release, with a headline under it.
Deriving the history from that instead of from a sidecar means it cannot
drift from what was actually released — the same reason `health` and
`design` are read from the engine rather than recomputed in the viewer.

Deliberately NOT from `git log`. Tags record every commit, including the
ones that fixed the previous commit; a CHANGELOG heading is the author's
own statement that something shipped, which is what "in broad strokes"
asks for.
"""
import os
import re
...
# form leaves every other repo with an empty history
# (REQ-CHANGELOGFORMS-1015):
#   ## plugin `vX.Y.Z` — YYYY-MM-DD   this repository, which
#                                     `check_versions.py` reads
#   ## [X.Y.Z] - YYYY-MM-DD           Keep a Changelog
#   ## vX.Y.Z - YYYY-MM-DD  /  ## X.Y.Z (YYYY-MM-DD)
# Both dash characters, because an em dash and a hyphen mean the same
# thing here.
_VERSION = r"v?[0-9]+\.[0-9][0-9A-Za-z.\-]*"
_ENTRY_RE = re.compile(
    r"^##\s+(?:plugin\s+`(" + _VERSION + r")`|\[(" + _VERSION + r")\]|("
    + _VERSION + r"))"
    r"\s*(?:[—–-]+\s*|\()(\d{4}-\d{2}-\d{2})\)?\s*$",
    re.M)
...
def _headline(body):  # implements: REQ-HISTORY-1003
    """The entry's first bold run, flattened to one line, or its first
    prose line.

    Every entry in this repo opens with a bold sentence naming what
    shipped; a consumer who does not write one still gets something,
    because a heading with no summary at all reads on the chart as a
    version that did nothing."""
    for raw in body.split("\n"):
        line = raw.strip()
        # `* ` with the space is a bullet; `**` is the bold headline every
        # entry opens with. Skipping on a bare `*` threw away the headline
        # of every entry and left the chart labelled with the second line
        # of the paragraph.
        ...
        # A bold run ending in a colon is a LEAD-IN to the list under it,
        # not a summary ("First feature release since `v1.0.0`.
        # Highlights:"). Taking it leaves the chart labelled with half a
...
def _weight(version):  # implements: REQ-HISTORY-1003
    """Sort key picking a month's landmark release: a major beats a minor
    beats a patch, and among equals the newer version wins.

    The month's FIRST release is not it — it is whatever happened to land
    first, often a patch on the month before. The month's LAST is not it
    either, for the same reason at the other end. What a reader means by
    "what happened in August" is its biggest step."""
...
def parse_changelog(text):  # implements: REQ-HISTORY-1003
    ...
    version that never shipped has no place on a timeline of what
    shipped."""
    out, marks = [], list(_ENTRY_RE.finditer(text))
    # Body runs to the next heading of ANY kind, dated or not, so an
    # undated entry's prose is never absorbed into the dated one above it.
...
def changelog_style(text):  # implements: REQ-RELEASECMD-1018
    """The heading form a CHANGELOG already uses — "plugin", "keep" or
    "bare" — so a new entry is written the way the file's author writes
    them. Keep a Changelog when empty."""
...
def entry_body(text, version):  # implements: REQ-RELEASEWORKFLOW-1019
    """The prose under `version`'s dated heading, or "" — the release
    notes."""
    stops = [m.start() for m in _HEADING_RE.finditer(text)] + [len(text)]
...
def read_history(root):  # implements: REQ-HISTORY-1003
    """Parsed CHANGELOG entries for `root` or its parent, or [] when
    there is none."""
...
def by_month(entries):  # implements: REQ-HISTORY-1003
    ...
    One row per calendar month, because 100 releases across four months
    is a wall of ticks and the question it answers is "what happened
    ...
    for e in sorted(entries, key=lambda x: (x["date"], x["version"])):
        ...
        row = months.setdefault(key, {
            "month": key, "count": 0, "first": e["date"],
        ...
        if (not row["landmark"]
                or _weight(e["version"]) > _weight(row["landmark"])):
