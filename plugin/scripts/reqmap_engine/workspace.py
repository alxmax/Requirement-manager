"""Workspace (the loaded repo), GateContext (what every rule reads),
the source-repo predicate and the test-link probe.
"""
import os, re
...
# `#[tokio::test]`. Used only to confirm a tested-by file holds
# tests — not to count.
...
# FuzzXxx — a private `func testHelper(` is NOT a test and must not
# satisfy a link.
_DEF_TEST_RE = re.compile(
    r"(?i:def\s+test\w*\s*\()|(?i:function\s+test\w*\s*\()|"
    r"func\s+(?:Test|Benchmark|Example|Fuzz)\w*\s*\(|"
    r"(?i:#\[\s*(?:[\w:]+::)?test\b)")
# The bare Jest/Mocha `it(` / `test(` call is too common a word to
# trust in prose or config (e.g. "it (the parser) returns None" in a
# .md), so it is honored ONLY in a JS/TS source file, where it is a
# genuine test idiom.
_CALL_TEST_RE = re.compile(r"\b(?:it|test)\s*\(", re.IGNORECASE)
_CALL_TEST_EXTS = (".js", ".ts", ".tsx", ".jsx", ".mjs", ".cjs")

# A stdlib-only Python suite often exposes no `def test…` — it drives
# its checks from a runnable entry point (`run` / `run_tests` /
# `main`) under an `if __name__ == "__main__"`
# guard, signalling pass/fail via the process exit code. Honored ONLY
# for a `tested-by` target (the author has already declared the file
# a test) and ONLY in a `.py` file, so a non-test module that merely
# defines a `main()` is never mistaken for a test elsewhere.
_PY_TEST_ENTRY_RE = re.compile(
    r"^\s*def\s+(?:run|run_tests|main)\s*\(", re.MULTILINE)
_PY_MAIN_GUARD_RE = re.compile(r"""if\s+__name__\s*==\s*["']__main__["']""")

# A shell suite declares a test as a bash function (`test_x() {`,
# `function test_x {`) or a bats case (`@test "…" {`) — none of
# which match the Python/JS/Go/Rust idioms above, so four real bash
# suites warned permanently in a consumer repo. The filename
# conventions are honored too: naming a file `*.test.sh` AND tagging
# it `tested-by` are two independent declarations that it is a test.
_SH_TEST_EXTS = (".sh", ".bash", ".zsh", ".bats")
_SH_TEST_NAME_RE = re.compile(
    r"(?:^|[._-])test[._-]|[._-]test$|^test[._-]|^tests?$", re.I)
...
def _test_link_problem(path):
    ...
    """Return a short reason a `tested-by` file fails the
    behavior-sync check, or '' when it is fine. A file that is
    missing, unreadable, or holds no recognizable test function means
    the link asserts coverage it does not have. Deterministic and
    warn-only — it never proves per-criterion coverage, only that
    real tests exist at the link target (per-AC mapping needs a
    per-AC tag, deferred)."""
    ...
    if (path.lower().endswith(".py")
            and _PY_TEST_ENTRY_RE.search(src)
            and _PY_MAIN_GUARD_RE.search(src)):
        return ""
    ...
    return ("contains no test function "
            "(def test.../func TestX.../#[test]/it()/bash test_x()/"
            "py run|main under __main__)")
...
class Workspace(object):  # implements: ARCH-RULES-059
    ...
    `reqs_dir` and `code_root` stay optional because two commands are
    defined for a caller that has neither: `health` skips the
    sections that need a code root rather than failing, and `next`
    does the same.
    """

    __slots__ = ("reqs", "members", "reqs_dir", "code_root",
                 "ac_cover", "level_cover", "_map_data")
    ...
    def map_data(self, root=".", members=None, with_design=True):
        """The assembled map document, built once per (members, root)
        and reused.
        `gate` asked for it twice — the RM027 freshness rule and the
        explicit `map --check` that follows — and each assembly
        re-ran the design review, the health record and the TODO
        parse (about a tenth of a gate's wall time).
        The cache is keyed on `with_design` too: a map built for a
        freshness check carries no design record and must never be
        the one a later write reuses."""
        members = self.members if members is None else members
        cached = self._map_data
        key = (root, with_design)
        if cached is None or cached[0] is not members or cached[1] != key:
            data = _assemble_map_data(self.reqs, members, self.reqs_dir,
                                      root, self.ac_cover,
                                      with_design=with_design)
            self._map_data = cached = (members, key, data)
        return cached[2]
    ...
    def load(cls, reqs_dir, code_root, cache=False):
        ...
        members, ac_cover, level_cover = scan_all(code_root, reqs_dir,
                                                   cache=cache)
...
class GateContext(object):
    ...
    """Everything a gate rule may read, computed once per run.
    `members` is the view `--since` may have narrowed; `full_members`
    is always the whole scan, because an existence check ("does an
    implements tag exist AT ALL") must never be answered
    from a filtered view."""
    # Set by `cmd_check` when a full scan already hashed every
    # member; any other caller of run_gate_rules leaves it None and
    # the member-drift rule hashes for itself. Defaulted here so a
    # second caller cannot trip over its absence.
    full_member_hashes = None
    # Set by `cmd_check` for the bare `gate` verb (ADR-0049): run
    # DEFAULT_RULES only.
    quiet = False
    ...
    def __init__(self, ws, since=None, full_members=None, update_lock=False):
        ...
        # the FULL scan, not the --since-narrowed one: whether this
        # repo has adopted `validated-against:` at all is a property
        # of the repo, not of one diff. Read narrowed, a push that
        # happened to touch one validation file switched the rule
    ...
    def in_scope(self, rid):
        """--since scoping: an already-true finding is reported only
        when the requirement's members are in the diff, or when
        there is no --since at all."""
        return rid in self.members or not self.since

    def roles(self, rid):
        """Tag roles a requirement carries, always over the FULL
        scan. A rule asks `--since` for its SCOPE (`in_scope`), never
        for its FACTS: the narrowed member set is missing every
        unchanged file, so reading a fact from it reports the
