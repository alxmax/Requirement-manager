"""Requirement files on disk: frontmatter, multi-block files,
load_requirements."""
import os, re, sys
...
def _scalar_value(v):  # implements: ARCH-PARSE-001
    """One frontmatter scalar value. If it opens with a matching quote,
    take the quoted span verbatim — a '#' inside is DATA, and any text
    after the closing quote (e.g. an inline comment) is dropped.
    Otherwise drop a ' #' / leading '#' comment, preserving an embedded
    '#' with no leading space (issue#123)."""
    v = v.strip()
...
def _parse_meta_lines(lines):
    # implements: ARCH-PARSE-001  # implements: REQ-PARSE-891
    """The frontmatter key/value reader: scalars, inline `[a, b]` lists,
    and the block form (`key:` then indented `- item` lines). Takes the
    lines between the fences, so it never has to know where the block
    began."""
    meta = {}
    i = 0
    while i < len(lines):
        ...
        if v.startswith("["):
            ...
            meta[k] = [x for x in
                      (_scalar_value(x) for x in inner.split(",")) if x]
        elif not v:
...
def parse_frontmatter(text):
    # implements: ARCH-PARSE-001  # implements: REQ-PARSE-891
    # implements: REQ-PARSE-892
    """Return (meta_dict, body). Minimal YAML: scalars, inline [a, b]
    lists, and the block form (`key:` then indented `- item` lines). An
    inline list missing its closing `]` is parsed leniently rather than
    silently kept as a literal string."""
...
# A requirement file may hold SEVERAL requirements, one per frontmatter
# block — a module in the DOORS sense: the architecture requirement,
# then the code requirements beneath it. A block begins at a `---` line
# immediately followed by `id:`. That lookahead is what makes the split
# unambiguous: a bare `---` is a markdown horizontal rule and a
# frontmatter close, both of which appear inside a body, and neither is
# followed by an id line.
...
def load_requirements(reqs_dir):
    ...
    """Every requirement in the directory, keyed by id:
    `{id: {meta, body, path, ...}}`. One file may hold several blocks
    ...
    for name in sorted(os.listdir(reqs_dir)):
        ...
        for _i, _blk in enumerate(split_requirement_blocks(text)):
            ...
            # only the FIRST block may fall back to the filename; a
            # later block without an explicit id is a malformed block,
            # not a second requirement named after the file. A file
            # preamble (REQ-MODULEFILE-056) can also land at index 0
            # when the real block 0 is preceded by prose — but a
            # preamble never starts with the frontmatter delimiter
            # '---' (parse_frontmatter's own test for "this text has
            # frontmatter"), so gating the fallback on that same test
            # keeps prose from minting a synthetic id that can collide
            # with (and silently shadow) the real block 0's own id.
            rid = meta.get("id") or (
