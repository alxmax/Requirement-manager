...
"""`sync --release`: cut the next planned version from the plan, in one
step.

The number comes from the plan: the lowest planned milestone above what
has already been declared (ADR-0040). The run writes nothing without
`--apply`; with it, the version files are bumped, the CHANGELOG gains
the dated entry listing the work planned on that version, and the plan
drops the milestone and its bars — a plan that keeps a shipped version
is a plan describing the past. Tagging is left to CI, which reads the
declared version back."""
import datetime
import json
import os
...
def _changelog_path(code_root):
    """The CHANGELOG `read_history` reads — the code root's, else its
    parent's — or the code root's path when there is none yet."""
...
def _released_work(reqs_dir, key):
    # implements: REQ-PLANADVANCE-1020
    """(raw plan, milestone name, label, [bar]) for the planned version
    equal to `key`."""
    ...
    milestones = (raw.get("milestones")
                  if isinstance(raw.get("milestones"), dict) else {})
...
def _insert_entry(text, entry):
    # implements: REQ-RELEASECMD-1018
    """`text` with `entry` placed newest-first: directly under
    `## [Unreleased]` when the file keeps one, so what was collected
    there becomes this release's notes; otherwise above the first
    `## ` heading; otherwise at the end."""
    lines = text.split("\n") if text else ["# Changelog", ""]
    for i, line in enumerate(lines):
        if line.strip().lower().startswith("## [unreleased]"):
            return "\n".join(
                lines[:i + 1] + ["", entry.rstrip("\n")] + lines[i + 1:])
    for i, line in enumerate(lines):
        if line.startswith("## "):
...
def _declared(reqs_dir, code_root):
    # implements: REQ-RELEASEWORKFLOW-1019
    """What CI releases: the declared version, whether its tag exists,
    and its notes."""
    ...
    name = tag_form(key)
    exists = bool((_git(["-C", code_root, "tag", "-l", name],
                        timeout=5) or "").strip())
...
def release_plan(ws, code_root, reqs_dir, version=True):
    ...
    wanted = (version if isinstance(version, str)
              else next_planned_version(reqs_dir, code_root))
    key = semver3(wanted) if wanted else None
    problems = []
    if key is None:
        problems.append(
            "no planned milestone above {} - plan one in "
            "_planning.json, or name "
            "the version: sync --release vX.Y.Z".format(
                plan["baseline"] or "v0.0.0"))
    elif base and key <= base[0]:
        problems.append("{} is not above {} ({})".format(
            tag_form(key), base[1], base[2]))
    errs, _ = run_gate_rules(
        GateContext(ws, full_members=ws.members, update_lock=False),
        strict=False)
    if errs:
        problems.append(
...
def apply_release(plan, code_root, reqs_dir):
    ...
    """Carry out a problem-free plan: bump the files, write the entry,
    advance the plan."""
    root = os.path.abspath(code_root)
    ...
    text = _read(path)
    if not any(semver3(e["version"]) == semver3(plan["target"])
...
def _print_plan(plan, applied):
    print("release {} (declared {}, baseline {})".format(
        plan["target"] or "-", plan["declared"] or "-",
        plan["baseline"] or "-"))
    ...
    print("applied." if applied else
          "dry run - nothing written; add --apply to release. Tagging "
          "is CI's job.")


def cmd_release(ws, code_root, reqs_dir, version=True, apply_it=False,
                 as_json=False):
    # implements: ARCH-RELEASE-072  # implements: REQ-RELEASECMD-1018
    """Print the release plan; with `apply_it`, carry it out. Exit 2
    when blocked."""
...
Every release, newest first. A release is a heading `## [X.Y.Z] - YYYY-MM-DD`
whose first line is a bold sentence saying what shipped; the Plan chart reads
these as its history.
`reqmap.py sync --release --apply` writes the heading for the next planned
version, with the work planned on it, and anything collected under `Unreleased`
becomes that release's notes.
...
def _default_branch(code_root):
    ...
    ref = (_git(
        ["-C", code_root, "symbolic-ref", "--short",
         "refs/remotes/origin/HEAD"], timeout=3) or "").strip()
    return ref.split("/", 1)[1] if "/" in ref else "main"


def release_workflow(code_root, reqs_dir):
    # implements: REQ-RELEASEWORKFLOW-1019
    """The workflow text for this repo, or None when the engine does not
    live inside it — a workflow cannot run an engine the checkout does not
    contain."""
    root = os.path.abspath(code_root)
    engine = os.path.join(
    ...
    try:
        ...
        # Windows: a path on another drive has no relative form, and it
        # is certainly not inside the checkout.
        return None
...
def _uses_github(code_root):
    return (os.path.isdir(os.path.join(code_root, ".github"))
            or "github.com" in (_git(
                ["-C", code_root, "config", "--get",
                 "remote.origin.url"], timeout=3) or ""))


def seed_release_files(code_root, reqs_dir):
    # implements: ARCH-RELEASE-072  # implements: REQ-CHANGELOGFORMS-1015
    # implements: REQ-RELEASEWORKFLOW-1019
    """Create what a repo needs to release and never overwrite it: a
    CHANGELOG when there is none, and the release workflow when the
    repo is on GitHub. Returns (created, notes)."""
    created, notes = [], []
    ...
    if _uses_github(code_root) and not os.path.exists(workflow):
        ...
        if text is None:
            notes.append(
                "no release workflow: the engine is not inside this "
                "repository, so "
                "CI could not run it. Vendor it under scripts/ and "
                "re-run init.")
        else:
