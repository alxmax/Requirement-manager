...
{members}
- {copied} case(s) copied here from {parent}; {unassigned} of the parent's cases went to no
  child because no group's subject appeared in their text, or more than one did.
- {parent} was not edited: its Description still holds this group's clauses, so deleting
  this file loses nothing. Once this child is confirmed, trimming the parent is yours to do.
"""
...
def _group_label(raw):  # implements: REQ-DECOMPOSE-994
    """The group label a flush-left line opens, or None. `**Label**` alone is one, and so is
    `**Label** (a note on it)`: a bold span opening a flush-left line that is not a bullet
    is a heading the author wrote. Reading only the bare form glued the second shape, label
    and all, onto the previous group's last clause — five groups became three children."""
...
def _case_raw_blocks(body):  # implements: REQ-DECOMPOSE-994
    ...
    copied case reads in the child exactly as it read in the parent. `_acc_blocks` folds a
...
def _plan_group_split(rid, r, reqs, reqs_dir, code_root, members):  # implements: REQ-DECOMPOSE-994
    ...
    plan["unassigned_case_labels"] = [b.get("label", "") for i, b in enumerate(blocks)
                                      if i not in owner]
    return plan
...
def _plan_and_report(targets, reqs, reqs_dir, code_root, members):
    ...
    Returns (plans, total_children, total_copied, total_unassigned)."""
...
def _write_group_split(plans, reqs, reqs_dir):
    # implements: ARCH-DECOMPOSE-050  # implements: REQ-DECOMPOSE-994
    """Write every plan's children; the parent is never opened for writing. Returns files
    written."""
    written = 0
    for plan in plans:
        if plan["refuse"]:
        ...
        for c in plan["children"]:
            ...
            with open(dest, "w", encoding="utf-8") as f:
                f.write(_render_group_child(c, rid, r["meta"], plan["unassigned_cases"]))
...
def cmd_decompose_groups(ws, only=None, apply_it=False, code_root=None):
    ...
    the children, returning 0.

    The parent is NEVER edited: each child is a copy of its group's clauses and cases.
    Moving the text instead once left 33 parents holding nothing but `see [[child]]`
    pointers, so rejecting a split — deleting its children — silently destroyed the
    contracts they had taken with a green gate. A copy makes every split undoable by
    deleting files. Trimming the parent once a child is confirmed is the author's edit,
    and RM036 reports any `[[ID]]` the trimmed Description is then left pointing at in
    vain."""
    ...
    print("\n{} child requirement(s) from {} parent(s); {} case(s) copied to a child, {} "
          "to none.".format(total_children, n_planned, total_copied, total_unassigned))
    ...
    print("The parent is not edited: its clauses and cases are copied, never moved.")
    if not apply_it:
        print("\nNothing written. Re-run with --apply; delete the children to undo.")
        return 0
    written = _write_group_split(plans, reqs, reqs_dir)
    print("\n{} file(s) written. Next: review each child, tag its members, then `reqmap.py "
          "sync`.".format(written))
    return 0
