"""`health` and `coverage`: the corpus coherence score and the
acceptance-coverage table."""
import json, os

from .git import _git
from .healthrows import EXEMPT_KEYS, _health_rows, _is_healthy
...
def _exempt_note(data):  # implements: REQ-HEALTH-968
    """', N with an exemption' for a score line, or '' when nothing is
    waived."""
    return (", {} with an exemption".format(data["exempt"])
           if data.get("exempt") else "")


def _link_sync_errors(reqs, members):
    # implements: ARCH-HEALTH-017  # implements: REQ-RULES-947
    """`gate`'s ERROR-level link-sync problems (dangling tags, enforced
    requirements with no `implements:` member) as message strings, for
    `health` — the same two rules the gate runs (RM001, RM006), read from
    the registry so the two commands cannot drift apart again (RM-6)."""
    # Imported here, not at the top: the gate's map-freshness rule embeds this
    # record (mapcmd -> health), and health runs two gate rules — mutual
    # by design.
    ...
    # honour `gate_exempt:` exactly as run_gate_rules does, or a requirement
    # the gate passes still counts as a link-sync error in `health` and the
    # committed map
...
def cmd_coverage(ws, as_json=False):
    ...
    # requirements dir holds spec files, not implementation files — excluded
    # from coverage. `_walk_files` already prunes it by realpath; this
    # abspath test also catches an SSOT dir reached through a symlink.
    reqs_abs = (os.path.normcase(os.path.abspath(reqs_dir))
               if reqs_dir else None)
    tagged_files = set()
    for mlist in members.values():
        for _role, fp, _ln in mlist:
            tagged_files.add(os.path.normcase(
    ...
    for fp, rel in _walk_code(code_root, reqs_dir):
        ...
        # carry a tag put the ratio's ceiling below 100% and named no file
        # the author could act on.
        if untaggable_by_design(rel):
            by_design += 1
            continue
        # Group by first path component (top-level directory or "." for
        # root files)
        parts = rel.split("/")
    ...
    for label in sorted(buckets):
        ...
        rows.append({"dir": label, "total": total, "tagged": tagged,
                    "pct": pct})

    if as_json:
        print(json.dumps({"rows": rows, "excluded_by_design": by_design},
                        indent=2))
        return 0
    ...
    print("\nTotal: {}/{} files tagged ({:>3}%)".format(
        tagged_all, total_all, pct_all))
    if by_design:
        print("({} file(s) excluded: they carry no tag by contract — "
              "decision records, "
              "CHANGELOG, LICENSE, issue templates. `gate --risk` skips "
              "the same set, so "
...
def _commits_since_reqs_touch(code_root, reqs_dir):
    ...
    The advisory "registry lag" signal: a large number means the registry
    has sat frozen while code raced ahead of it — the exact 18-day-freeze
    condition that let a money value drift with no requirement update.
    Returns None (not 0) when unmeasurable — git missing, `code_root` not
    a git worktree, or `reqs_dir` has no commit in history — so the
    reading is absent rather than falsely 0. Read-only; never a gate,
    never enters the score."""
    # reqs_dir must resolve against the CALLER's cwd, not against code_root
    # — `git -C code_root` changes where the pathspec is resolved, so a
    # relative reqs_dir (e.g. `--code ..` from `plugin/`) would silently
    # look for `../requirements` instead of `../plugin/requirements`.
    # Mirrors the abspath(p) pattern `untracked_locks` already uses for
    # the same reason (ARCH-CHECK-006).
    sha = (_git(["-C", code_root, "log", "-1", "--format=%H", "--",
                os.path.abspath(reqs_dir)],
                timeout=5) or "").strip()
    if not sha:
        return None
    cnt = _git(["-C", code_root, "rev-list", "--count",
              "{}..HEAD".format(sha)], timeout=5)
    try:
...
def _requirement_health_flags(rid, r, members, lock, satisfied):
    ...
    """The per-requirement booleans `_health_record`'s loop aggregates
    into counters."""
    ...
    return {"status": status, "has_impl": has_impl,
            "has_test_member": has_test_member,
            "test_exempt": bool(m.get("test_exempt")), "has_test": has_test,
            "is_confirmed": is_confirmed, "covered": covered,
            "open_now": open_now,
...
def _reviewed_score(confirmed, drafts, healthy):
    # implements: ARCH-HEALTH-017  # implements: REQ-HEALTH-968
    """The reviewed-subset score (REQ-REVIEWEDSCORE-109) and its
    denominator, or (None, confirmed) when there is nothing for it to add."""
    # Reviewed-subset score (read-only, ADDITIVE). `score` counts every
    # requirement, and a `draft` can never be green because the first axis
    # is status `confirmed` — so each draft caps `score` by construction
    # until someone confirms it. That makes the headline unable to tell
    # "rotting" from "not reviewed yet": a repo that runs `init` over
    # legacy code gets hundreds of drafts and a near-zero score that no
    # amount of care moves. This second number scores only the reviewed
    # part, so the two readings are separable. `score` itself is NOT
    # redefined — CASE-2 binds an all-draft corpus to zero, and every
    # consumer badge already reads `score`.
    # Absent (not zero) when nothing has been reviewed, like `untagged`
    # above: 0 of 0 is not 0%, and a consumer's schema must not gain a
    # meaningless key.
    # implements: REQ-REVIEWEDSCORE-109
    # Emitted only when drafts and reviewed requirements BOTH exist: with
    # no reviewed requirement it would be 0 of 0, and with no draft it
    # would restate `score` under a second name, which is how a
    ...
    # and it would cap the score forever.
    # Invisible in THIS repo (all 72 non-drafts are `confirmed`, so the
    # two readings coincide at 100), which is exactly why it is pinned by
    # a test instead of by luck.
    reviewed_total = confirmed  # implements: REQ-REVIEWEDSCORE-109
    reviewed_score = (round(100 * healthy / reviewed_total)
                      if (reviewed_total and drafts) else None)
    return reviewed_score, reviewed_total


def _health_record(reqs, members, reqs_dir, with_rows=False):
    # implements: ARCH-HEALTH-017  # implements: REQ-HEALTH-968
    """The corpus coherence snapshot as a record, with no printing and no
    code root: the headline `score` plus the component counts behind it.
    Split out of `cmd_health` so the map can carry the same numbers the
    console prints instead of a viewer recomputing them in JavaScript —
    two definitions of one score is how the CLI and the UI come to
    disagree about how the repo is doing.
    ...
    `with_rows` adds `unhealthy` and `exempt_ids` for the map's viewer.
    It is off by default so `health --json`, a badge payload, stays one
    small object."""
    total = len(reqs)
    ...
    for rid, r in reqs.items():
        ...
        untested += (f["has_impl"] and not f["has_test_member"]
                    and not f["test_exempt"])
        open_intent += f["open_now"]
        drifted += f["is_drifted"]
        healthy += _is_healthy(f)
    # implements: REQ-HEALTH-858
    # A `deprecated` requirement is out of service and can never be green,
    # so counting it capped the headline forever: this repo read 97/100
    # with nothing wrong, 8 retired requirements in the denominator. The
    # reviewed score already excluded them.
    scored = total - retired
    score = round(100 * healthy / scored) if scored else 0
    # implements: REQ-HEALTH-968
    # A waiver is part of the score's meaning: 100/100 over twenty
    # exemptions says less than 100/100 over none, and a reader of the
...
def _health_gather_signals(reqs, code_root, reqs_dir, data):
    ...
    """Layer the code-root-dependent signals (untagged files, registry lag,
    design score, roadmap drift) onto `data`.
    Returns (data, untagged, lag, design)."""
    ...
    # touched — a frozen registry while code moves ahead. Absent (not 0)
    # when unmeasurable (no git / no code root), like `untagged`.
    # implements: ARCH-REGISTRYLAG-035
    lag = (_commits_since_reqs_touch(code_root, reqs_dir)
          if code_root else None)
    if lag is not None:
        data["commits_since_req_touch"] = lag  # implements: REQ-REGISTRYLAG-904
    # Roadmap signals (read-only): does TODO.md still track what shipped,
    # and does every section heading actually parse as a milestone. Absent
    # (not empty) when the repo has no TODO.md, so a repo that does not
    # keep one sees nothing. implements: ARCH-ROADMAP-038
    ...
    if roadmap is not None:
        ...
        if unmapped:
            data["roadmap_unmapped"] = {
                "shipped": roadmap["newest_shipped"],
                "requirements": newest_req}
        if roadmap["unversioned_headings"]:
            data["roadmap_unversioned_headings"] = (
                roadmap["unversioned_headings"])
    # The plan's own gaps, counted here and named by `next`'s Plan bucket
    # from the one computation — `gate --risk` prints the bucket and
    # `gate --risk --json` is this record, so a signal on one surface and
    # not the other is how the two come to disagree about what is left to
    # do. Absent (not 0) with no ROADMAP.md, like `untagged`: a repo that
    # keeps no plan gains no key. implements: REQ-PLANGAPS-1033
    ...
    misaligned = (version_alignment_lines(reqs_dir, code_root)
                 if reqs_dir else [])
...
def _health_badge_payload(data):
    # implements: ARCH-HEALTH-017  # implements: REQ-HEALTH-857
    # implements: REQ-HEALTH-858  # implements: REQ-HEALTH-859
    """The shields.io badge payload for `--badge`, red whenever `gate`
    itself would fail with a link-sync error a green score would
    otherwise hide."""
...
def _print_health_report(data, design, untagged, lag, headline_only):
    ...
    if headline_only:
        # `next` opens with the score and then lists what to do about it;
        # the component breakdown below would push the actionable part
        # off the first screen.
        # The design score rides along: the other half of "how is this repo
        # doing".
        if design is not None:
            print("Design pass-rate:   {}%  ({}/{} source files with no "
                  "candidate)".format(design["score"], design["clean_files"],
                                      design["files"]))
        return
    # Say what the headline cannot: a draft caps `score` by construction,
    # so a low reading over a draft-heavy corpus means "not reviewed yet",
    # not "rotting". Printed only when drafts actually pull the two
    # numbers apart.
    if reviewed_score is not None:
        print("  reviewed only:      {}/100  ({}/{} confirmed, {} not "
              "confirmed yet)".format(
    ...
    if orphans:
        print("  orphans (confirmed, no code):     {}".format(orphans))
    if untested:
        print("  untested (code, no tests):        {}".format(untested))
    if open_intent:
        print("  open verify-intent:               {}".format(open_intent))
    if drifted:
        print("  drift (contract changed vs lock): {}".format(drifted))
    if gate_errors:
        print("  gate link-sync errors (not clean):{}".format(gate_errors))
    if untagged:
        print("  untagged code (no requirement):   {}".format(
            len(untagged)))
    if lag:
        print("  commits since requirements touched:{}".format(lag))
    if design is not None:
...
def cmd_health(ws, as_json=False, as_badge=False, headline_only=False):
    # implements: ARCH-HEALTH-017  # implements: REQ-HEALTH-857
    # implements: REQ-HEALTH-858  # implements: REQ-HEALTH-859
    """Print a corpus coherence snapshot: a headline score plus component
    counts. The score is transparent — the percentage of requirements
    green on EVERY axis (confirmed, has an `implements` member,
    tested-or-`test_exempt`, no open
    ...
    data, untagged, lag, design = _health_gather_signals(
        reqs, code_root, reqs_dir, data)
    if as_badge:
