...
from .candidates import (
    SYS_PLACEHOLDER_ID, _assign_arch_ids, _draft_id, _file_facts
)
from .parse import parse_frontmatter
from .scan import _walk_files, read_source_text
from .tags import (
    PROSE_EXTS, TAG_RE, _is_code_file, _is_test_path, classify_prose,
    tag_comment_for, tag_insert_index, tagged_files
)


def _prose_facts(src):
    # implements: ARCH-PROSE-024  # implements: REQ-PROSE-901
    """(title, [headings]) from markdown/HTML prose, for a draft scaffold.
    Title: markdown frontmatter `title:`, else first `# ` H1, else
    <title>/<h1>. Headings: markdown `## ` H2 lines, else <h2>. Returns
    (None, []) when absent. The scaffold lists headings as an authoring
    hint — never the contract."""
    ...
    for line in body.splitlines():
        ...
        if title is None:
            ...
            if m:
                ...
                # no continue: a line may carry both <title> and <h2>
                # (see test_html_title_and_h2)
    ...
    # A prompt corpus (fabric: 255 files) writes every section as `# `:
    # with no H2 at all, the later H1s ARE the sections, and the hint
...
def _write_sys_placeholder(reqs_dir, arch_ids):
    ...
    A stakeholder need is not in the source — nothing in a repository says
    why a user wants the thing — so the engine refuses to guess one and
    mints a node whose title says so. Skipped only when this file already
    exists (a second `init` never overwrites it). A hand-named need
    elsewhere in the corpus does NOT suppress it: every architecture draft
    is written pointing at this id, and the engine will not guess which
    real need those drafts satisfy — that is the author's edit, and the
    ...
    with open(dest, "w", encoding="utf-8") as f:
        f.write("---\nid: {}\nstatus: draft\nlevel: system\nlayer: need\n"
                "owner: auto\nlevel_source: auto\n---\n\n"
                "# NAME THIS NEED\n\n"
                "> The engine cannot read a stakeholder need out of source "
                "code, so it "
                "left this hole rather than invent one. Replace the title and "
                "the clause "
...
def _write_arch_drafts(reqs_dir, by_dir, id_of):
    ...
    Returns the ids written, newest-corpus-first order irrelevant. Each is a
    proposal:
    `status: draft`, `owner: auto`, `level_source: auto`, and a title that
    names the
    ...
    for rel_dir in sorted(by_dir):
        ...
        with open(dest, "w", encoding="utf-8") as f:
            f.write("---\nid: {aid}\nstatus: draft\nlevel: architecture\n"
                    ...
                    "structural "
                    "signal — the directory `{rel_dir}` — and a directory is "
                    "not a "
                    "capability. Rename this to the thing these {n} behaviour "
                    "group(s) "
                    "together let a user do, merge it with a sibling, or "
                    "delete it and "
                    ...
                    "  When   TODO\n"
                    "  Then   TODO\n".format(
                        aid=aid, sys=SYS_PLACEHOLDER_ID,
                        label=rel_dir or "root",
                        rel_dir=rel_dir or ".", n=len(kids),
                        bullets="\n".join(
                            "- TODO: one obligation this capability owes. "
                            "[[{}]]".format(k)
                            for k in kids)))
        written.append(aid)
    return written
...
def _write_prose_draft(dest, cap, rel, fn, src, arch_id):
    ...
    """Write one DRAFT .md for a prose capability file; returns the review
    label."""
    ...
    with open(dest, "w", encoding="utf-8") as f:
        f.write("---\nid: {cap}\nstatus: draft\nlevel: code\n"
                ...
                "## Description\n"
                "Every bullet below is binding.\n"
                "<!-- Name the subject, write in present tense, one "
                "statement per "
                ...
                "- TODO: which source sections are normative vs "
                "illustrative?\n\n"
                "## Cases (= tests)\n"
                ...
                "- {rel}\n\n"
                "**Source sections detected (authoring hint, not the "
                "contract)**\n"
                "{hint}\n".format(
...
def _write_code_draft(dest, cap, rel, fp, src, arch_id):
    # implements: ARCH-EXTRACT-008  # implements: ARCH-PROSE-024
    # implements: REQ-EXTRACT-849  # implements: REQ-EXTRACT-850
    """Write one DRAFT .md for a code file; returns the review label. `fp`
    is the file's full path (dirpath + fn folded into one, to keep the
    ...
    with open(dest, "w", encoding="utf-8") as f:
        ...
        f.write(f"---\nid: {cap}\nstatus: draft\nlevel: code\n"
                ...
                f"<!-- Name the subject, write in present tense, one "
                f"statement per "
                ...
                f"- TODO: anything that looks like an accident (swallowed "
                f"error, magic "
                f"constant, dead branch) — intended, or a bug to fix?\n\n"
                f"## Cases (= tests)\n"
                f"- characterization: current behavior captured, "
                f"correctness UNVERIFIED\n\n"
...
def _tag_the_source(fp, rel, cap, is_test):
    ...
    """Write the membership tag for `cap` into its own source file. True
    when written.

    Without this, `init` wrote a stub and left the source untagged, so
    the file stayed in the untagged bucket forever and `gate --risk`
    kept proposing the `init` that had already run — while the stub it
    produced had zero members. A consumer deleted eleven such orphans by
    hand before anyone noticed the loop.

    Reversible by `init --wipe`, which strips exactly the markers
    `tag_comment_for` writes. Skipped, never forced, when the file type
    has no comment form, when it is already tagged, or when it cannot be
    read as text — a file the scan cannot decode must not be rewritten
    from a half-decoding."""
    ...
    # `read_source_lines` normalises line endings (that is what a tag's
    # line NUMBER means); a rewrite must not, or every CRLF file silently
    # converts to LF. `splitlines(keepends)` round-trips exactly, so the
    # bytes outside the inserted line are untouched.
...
def cmd_extract(ws):
    ...
    # the same definition `plan` reports (REQ-PLANTAGGED-1005)
    tagged = tagged_files(members)
    proposed, tagged_n, used = 0, 0, set()
    # pending writes: (fp, rel, is_prose, dest, cap, rel_dir)
    jobs = []
    ...
    # rel dir -> [code-level draft ids], for the ARCH rung
    by_dir = {}
    for fp, rel, is_prose, dest, cap, rel_dir in jobs:
        dirpath, fn = os.path.dirname(fp), os.path.basename(fp)
        src, _problem = read_source_text(os.path.join(dirpath, fn))
        # undecodable: draft the stub from its path, not from rubbish
        src = src or ""
        ...
        print(f"{review:14} {cap}  <- {rel}"
              f"{'' if linked else '   (source not tagged)'}")
    # The two rungs above the code level. Written last, so they know
    # their children.
    arch_ids = _write_arch_drafts(reqs_dir, by_dir, id_of)
    n_sys = _write_sys_placeholder(reqs_dir, arch_ids)
    if arch_ids:
        print(f"\n{len(arch_ids)} architecture draft(s) proposed from "
              f"directory names, and "
              f"{n_sys} system placeholder. Both carry `level_source: "
              f"auto` — the engine "
              f"invented them and a directory is not a capability. "
    ...
    if proposed:
        print(f"{tagged_n} of {proposed} source file(s) were linked to "
              f"their draft in place "
              f"(`init --wipe` removes those tags). The rest have no "
              f"comment form the "
              f"engine writes, or were already linked — tag them by "
              f"hand or they stay in "
              f"the untagged bucket.")
    return 0


def _observed_surface(facts, limit=12):
    # implements: ARCH-EXTRACT-008
    """Authoring hint for a code draft's Context/Current-implementation
    group: the module docstring's first line and the top-level
    signatures `plan` already knows how to read. Empty string when the
    language has no parser. Non-binding by construction — it lives
    under Context, never in the Contract, so a promoted draft still
    needs an authored contract."""
    sigs = list(facts.get("signatures") or [])
    doc = (facts.get("docstrings") or {}).get("module")
    if not sigs and not doc:
        return ""
    lines = ["", "Observed surface (auto, non-binding — an authoring "
              "hint, not the contract):"]
    if doc:
