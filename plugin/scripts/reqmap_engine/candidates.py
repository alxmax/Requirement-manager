"""`plan`: read-only capability-extraction plan from file facts (Python, JS,
Markdown)."""
...
# Stage 1 of AI extraction: gather the raw material an authoring step (a human
# or an LLM agent) needs to write a real, capability-level requirement.
# READ-ONLY — emits a JSON plan, writes NO .md, so it cannot repeat extract's
# empty-stub failure. Languages `plan` can read FACTS from (docstrings,
# signatures). Every scannable code file is a candidate regardless — three
# evidence runs (zlib: 0 candidates for 94 C files, gin: none for 99 Go files,
# awesome-compose: none for 35 Dockerfiles) showed a plan that silently omitted
# most of what `draft` then produced.
CANDIDATE_EXTS = (
...
def _class_method_signatures(node):
    ...
    """Public method signatures of a class body, as `def Class.method(args)`
    strings.
...
def _js_facts(src):
    # implements: ARCH-CANDIDATES-009  # implements: REQ-CANDIDATES-826
    """Best-effort JS/TS facts via regex (no stdlib JS parser): the leading
    block comment as the module doc, and top-level function/binding names.
    Imports are not resolved for JS in v1 (the agent and _capmap.json fill
    that gap)."""
    facts = {"signatures": [], "docstrings": {}, "imports": []}
    # Leading block comment via plain string scan over a capped prefix — NOT a
    # regex. The old `/\*+(.*?)\*/` backtracks O(n^2) on a file opening with a
    # long run of `*` (a DoS on `candidates`); str.find is linear and cannot
    # backtrack. The leading `*`s of `/***` are stripped by the per-line
    # `.strip(" *")` below.
    ...
    if head_src.startswith("/*"):
        ...
        if close != -1:
            head = [ln.strip(" *")
                    for ln in head_src[2:close].strip().splitlines()
                    if ln.strip(" *")]
            if head:
                facts["docstrings"]["module"] = head[0][:200]
    names = re.findall(
        r"(?:export\s+)?(?:async\s+)?function\s+([A-Za-z_$][\w$]*)", src)
    names += re.findall(
        r"(?:export\s+)?(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=", src)
    facts["signatures"] = list(dict.fromkeys(names))   # dedupe, keep order
    return facts


def _md_facts(src):  # implements: ARCH-CANDIDATES-009
    """Best-effort capability facts from a Markdown prompt/spec file (no
    parser): the first H1 (`# `) is the title, the first blockquote (`>`)
    AFTER that H1 is the intent, and each `## ` H2 heading is a
    structural-signature line.
    Free prose is never hashed — these facts only seed a human-authored
    requirement (Stage 2); the binding hash anchors on the authored
    Contract+Acceptance, like any code requirement."""
    facts = {"signatures": [], "docstrings": {}, "imports": []}
...
def _file_facts(path, rel):
    # implements: ARCH-CANDIDATES-009  # implements: REQ-CANDIDATES-826
    src, _problem = read_source_text(path)
    if not src:   # unreadable, empty, or not decodable as text: no facts, and
        # LOC 0 — a UTF-16 file used to report twice its real line count here
        # (`errors="ignore"` turned its NULs into dropped bytes, not dropped
        # lines).
    ...
    if rel.endswith(".py"):
        ...
        facts = _js_facts(src)
    # a candidate the engine has no parser for: still listed, facts empty
    else:
        facts = {"signatures": [], "docstrings": {}, "imports": []}
...
def _load_capmap(reqs_dir):
    # implements: ARCH-CANDIDATES-009  # implements: REQ-CANDIDATES-827
    """Optional `requirements/_capmap.json`: a hand-authored capability
    grouping, authoritative when present. Shape: {"capabilities": [{id, layer,
    files:[...]}]} (a bare list is also accepted). Returns []; fail-open on
    absent/unreadable."""
    try:
        with open(os.path.join(reqs_dir, "_capmap.json"),
                  encoding="utf-8") as f:
            data = json.load(f)
            ...
            if isinstance(c, dict) and c.get("id") and c.get("files"):
                out.append({"id": c["id"], "layer": c.get("layer"),
                            "files": [f.replace(os.sep, "/")
                                      for f in _as_list(c["files"])]})
    return out


def _mint_cap_id(rel):
    # implements: ARCH-CANDIDATES-009  # implements: REQ-CANDIDATES-827
    """A TAG_RE-valid suggested id from a path stem (Stage 2 may rename it)."""
    slug = re.sub(
        r"[^A-Z0-9]+", "-", os.path.splitext(rel)[0].upper()).strip("-")
...
def _collect_files(code_root, reqs_dir, md_globs=None):
    ...
    for fp, rel in _walk_files(code_root, reqs_dir):
        ...
        if _is_code_file(fn) and not fn.endswith(PROSE_EXTS):
            out.append(rel)
        elif fn.endswith(".md") and any(
                fnmatch.fnmatch(rel, g) for g in md_globs):
            out.append(rel)
    return out
...
def _minted_groups(files, facts_by_file, reqs_dir, reqs):
    ...
    for entry in _load_capmap(reqs_dir):
        ...
        if present:
            groups.append({"id": entry["id"], "layer": entry.get("layer"),
                            "files": present})
...
def _test_by_stem_index(files):
    ...
    `_build_candidates` is O(1) per stem instead of rescanning `files` per
    group."""
    test_by_stem = {}
    for r in files:
        b = os.path.basename(r)
        if b.startswith("test_"):
            test_by_stem.setdefault(
                os.path.splitext(b)[0][len("test_"):], []).append(r)
    return test_by_stem


def _build_candidates(groups, facts_by_file, stem_of, group_id_of_file,
                       test_by_stem, tagged):
    # implements: ARCH-CANDIDATES-009  # implements: REQ-CANDIDATES-826
    """One candidate dict per group: signatures, docstrings, imports, deps,
    tested_by."""
    cands = []
    for g in groups:
        ...
        tested_by = sorted(
            t for stem in my_stems for t in test_by_stem.get(stem, ()))
        ...
        cands.append({  # implements: REQ-CANDIDATES-827
            ...
            "docstrings": docs, "signatures": sigs[:60],
            "imports": sorted(imps),
            "depends_on": deps, "tested_by": tested_by, "loc": loc,
            "existing_req": existing,
            "split_candidate": loc > cfg.SPLIT_LOC_THRESHOLD,
            "is_test": bool(g["files"]) and all(_is_test_path(f)
                        for f in g["files"]),
        })
    return cands
...
def _annotate_fanin(cands):
    # implements: ARCH-CANDIDATES-009  # implements: REQ-CANDIDATES-827
    """Mutate cands in place: importer_count and suggested_layer (bus vs
    feature), both derived from how many other candidates depend_on each one."""
    fanin = {}
    for c in cands:
    ...
    for c in cands:
        ...
        c["suggested_layer"] = (
            c.pop("_layer")
            or ("bus" if n >= cfg.BUS_FANIN_THRESHOLD else "feature")
        )
...
def _annotate_pyramid(cands):
    ...
    """Mutate cands in place with the rung and architecture parent `init` would
    mint, and return the upper rungs as `{"architecture": [...], "system": <id
    or None>}`.

    `init` writes three rungs (ADR-0036) and the plan reported none of them, so
    the only way to see the pyramid a run would produce was to let it write
    every file first. A candidate that is already linked in code is not drafted,
    so it gets no rung — stating one would describe a write that will not
    happen."""
    dirs = set()
    for c in cands:
        ...
        if drafted:
            dirs.update(
                os.path.dirname(f).replace(os.sep, "/") for f in c["files"])
    id_of = _assign_arch_ids(dirs)
    for c in cands:
        first = c["files"][0] if (c["level"] and c["files"]) else None
        c["arch_id"] = (
            id_of[os.path.dirname(first).replace(os.sep, "/")]
            if first else None
        )
        # DISCLOSURE, not unification (REQ-PLANDRAFTID-1010): `suggested_id`
        # is the group-level name an author may adopt, `draft_id` is the id
        # the write path will actually mint for this file. They differ on
        # purpose — `DRAFT-` is the marker that a requirement is an
        # unreviewed auto-draft — so the plan states both rather than
        # renaming either.
        c["draft_id"] = _draft_id(first) if first else None
    arch = sorted(set(id_of.values()))
    # the apex is written only when there is an architecture rung to hang
    # it under
    return {"architecture": arch,
           "system": SYS_PLACEHOLDER_ID if arch else None}
...
def cmd_candidates(ws, out, md_globs=None):
    ...
    `md_globs` opts non-code `.md` files (prompts, specs) into discovery —
    advisory only, never auto-written; a human authors + confirms each
    into the SSOT."""
    reqs = ws.reqs
    members, reqs_dir, code_root = ws.members, ws.reqs_dir, ws.code_root
    files = _collect_files(code_root, reqs_dir, md_globs)
    facts_by_file = {rel: _file_facts(os.path.join(code_root, rel), rel)
                     for rel in files}

    # file -> the requirement it is already linked to
    tagged = tagged_files(members)

    # depends_on is resolved by matching an import name to a file STEM. Known
    # limitation (Stage-1 heuristic): an import that shadows a stdlib/3rd-party
    # name (e.g. `import json` next to a local json.py) or collides with a
    # same-basename file in another dir can yield a false edge — the Stage-2
    # author prunes these.
    stem_of = {os.path.splitext(os.path.basename(r))[0]: r
              for r in files}  # for depends_on
    ...
    plan = {
        ...
        # the three rungs `init` would write, before it writes them
        # (REQ-PLANLEVEL-1006)
        "pyramid": pyramid,
        # surfaces the unfilled-plan gap so an advisory plan nobody authored
        # cannot masquerade as coverage (with_existing_req = candidates
        # already tagged in code)
        "coverage_summary": {"total_candidates": len(cands),
                             "with_existing_req": authored},
        "lineage_note": ("A generated-from/implements tag records "
                         "authoring lineage only; it "
                         "does NOT mean the requirement auto-tracks later "
                         "edits to the source "
                         "file. Re-touch the requirement's "
                         "Contract+Acceptance when the source's "
                         "behavior changes."),
        "bus": sorted(c["suggested_id"] for c in cands
                     if c["suggested_layer"] == "bus"),
        "candidates": cands,
    }
...
# These live HERE, one layer below the writer, because `--plan` must be
# able to say what `init` will write without importing it (draft.py
# imports candidates.py, never the other way round). Naming only: nothing
# below writes a file.
...
def _arch_slug(rel_dir):
    # implements: ARCH-EXTRACT-008  # implements: REQ-EXTRACT-981
    """Last two path segments — the spec's grouping key, not a unique id."""
    parts = [p for p in rel_dir.replace(os.sep, "/").split("/")
            if p not in ("", ".")]
...
def _assign_arch_ids(rel_dirs):  # implements: REQ-EXTRACT-981
    ...
    not collapse onto one node — the second takes `-002`, `-003`, … so the
    pyramid keeps one ARCH row per directory and every code draft has a
    parent that exists."""
    claimed, out = set(), {}
...
def _draft_id(rel):
    ...
    slug = re.sub(r"[^A-Z0-9]+", "-",
                 os.path.splitext(rel)[0].upper()).strip("-")
