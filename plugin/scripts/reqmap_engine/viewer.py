"""_map.html: the vendored single-file viewer, and the viewer fixture sync
check."""
import json, os
...
def _vds_parse_baked(path):
    """{id: [contract strings]} from the viewer's `baked.json` fixture, or None
    when the file is missing, unreadable or not a JSON list.

    An entry marked `demoOnly: true` is skipped. The fixture INVENTS a fake
    orphan and a fake deprecated capability so the Risk and Problems tabs have
    something to show with no engine present; those ids cannot exist in any
    registry, so comparing them reported permanent drift about data doing its
    job. An UNMARKED id missing from the registry still reports, because that is
    a requirement renamed out from under the fixture."""
    try:
        with open(path, encoding="utf-8") as f:
            data = json.load(f)
    except (OSError, ValueError):
        # ValueError covers UnicodeDecodeError and bad JSON
        return None
    if not isinstance(data, list):
        return None
    return {
        e["id"]: [str(c) for c in e.get("contract") or []]
        for e in data
        if isinstance(e, dict) and isinstance(e.get("id"), str)
        and not e.get("demoOnly")
    }


def check_viewer_data_sync(baked_path, map_nodes):
    # implements: ARCH-VIEWER-007
    """Compare the viewer's hand-authored fallback fixture,
    `app/src/lib/baked.json`, against the live registry (map_nodes: [{"id":...,
    "contract":[...]}, ...]). Returns a sorted list of requirement IDs where the
    two disagree — a baked id missing from the live registry, or a
    whitespace-normalized mismatch in its `contract` bullets. Warn-only. The
    fixture was a JavaScript literal read by a bracket-counting scanner; as JSON
    it is read as data, so no bullet text can confuse the reader. Returns None
    (not []) when the file is missing, unreadable or not a JSON list —
    ...
    drift = [
        rid for rid, contract in baked.items()
        if rid not in live
        or _vds_normalize(contract) != _vds_normalize(live[rid])
    ]
    return sorted(drift)
