...
def _risk_score(meta):  # implements: ARCH-NEXT-013  # implements: REQ-NEXT-885
    """Extract's per-file risk hint (0-3) from frontmatter, or 0 when
    absent / unparseable. Used only to float REVIEW-flagged drafts to
    the top of a bucket — never to gate. Hand-authored requirements
    have no `risk:` field -> 0."""
    try:
...
_PRIORITY_ORDER = {"must-have": 0, "should-have": 1, "could-have": 2,
                   "wont-have": 3}


def _recorded_members(reqs_dir, ids):  # implements: ARCH-NEXT-013
    """{id: first member loc} from the committed `_map.json`, for those
    of `ids` whose node records a member there. The narrow default scan
    (no --code) cannot see a member outside its root, so a requirement
    lands in Orphans while the committed map proves it has code — this
    is what turns that into a hint instead of a puzzle. Fail-open ({})
...
def _plan_gaps(reqs, code_root, reqs_dir):
    ...
    """The horizon work `next` names beside the corpus buckets, as
    (kind, text) pairs: open `Now`/`Next` items no bar schedules, open
    `Later` items with no `unpark:`, and items naming a `req:` the
    corpus does not hold.

    Empty — never a complaint — when the repo keeps no `ROADMAP.md`,
    which is most of them. These three already existed as one line
    each in `sync` and `gate --audit`; what they answer is "what is
    next", so they belong on the screen that asks it. Nothing here
    reads the code, so the whole thing costs one small file read."""
    # Imported at call time, not at the top: `mapdata` reads
    # `_risk_signals` from this module, so a module-level import would
    # close the cycle. The engine's second documented one, after
    # `health._link_sync_errors` -> `rules`.
...
def _print_plan_bucket(gaps, show_all, top_n):
    ...
    for kind, name in (gaps if show_all else gaps[:top_n]):
        print("  {:<16} {}".format(
            kind, name[:70] + ("..." if len(name) > 70 else "")))
    if not show_all and len(gaps) > top_n:
        print("  ... {} more — run `reqmap.py gate --risk --all`".format(
            len(gaps) - top_n))
    print("  -> The plan and the corpus disagree. Give a `Now`/`Next` "
          "item dates in "
          "`_planning.json`, an `unpark:` condition to a `Later` one, "
          "or point a `req:` "
...
def _next_pending(reqs, members, code_root, reqs_dir):
    ...
    if total == 0:   # distinguish "nothing set up yet" from "all clean"
        print("No requirements yet. Run `reqmap.py init` to bootstrap "
              "from existing "
              "code, or write requirements/AREA-NAME-NNN.md to author one.")
        return None
    confirmed = sum(1 for r in reqs.values()
                    if r["meta"].get("status") == "confirmed")
    tested = sum(1 for rid in reqs if any(
        role == "tested-by" for role, *_ in members.get(rid, [])))
    # `unreviewed` = draft + baseline, the same population the "Drafts
    # to review" bucket holds; the header used to count `draft` only
    # and disagree with the bucket beneath it.
    unreviewed = sum(1 for r in reqs.values() if r["meta"].get(
        "status", "draft") in ("draft", "baseline"))
    print("{} requirement(s) · {} confirmed · {} tested · {} "
          "unreviewed\n".format(total, confirmed, tested, unreviewed))
    ...
    # Action buckets, MOST-URGENT FIRST: an unimplemented contract
    # outranks an unreviewed draft. Each bucket is shown and truncated
    # independently, so a high-priority bucket is never hidden below a
    # long low one.
    PLAN = [
    ...
    pending = [(sig, label,
               sorted(buckets[sig],
                      key=lambda x: (_priority_ord(x[0]), -x[1], x[0])))
               for sig, label in PLAN if buckets.get(sig)]
    untagged = _scan_untagged(code_root, reqs_dir) if code_root else []
    # Granularity advisory: requirements with many ACs covering
    # disjoint behaviors. Shares `_oversize` with lint's
    # `ac-count-high` check (same threshold, same LINT_STATUSES
    # scoping, same `lint_exempt` honoring) so `next` and `lint` can
    # never report a different set for the same corpus.
    oversize = sorted(
    ...
    # The other direction of the same concern: covering the code with
    # FEWER requirements. Granularity above says "this one does too
    # much"; this says "these say the same thing".
    redundant = _redundant_groups(reqs)
    # The horizon, read from the plan files rather than from the
    # corpus: work the ROADMAP says is open and the plan does not
    # schedule. implements: REQ-PLANGAPS-1033
...
def _print_pending_bucket(sig, label, ids, reqs_dir, disp):
    # implements: ARCH-NEXT-013  # implements: REQ-NEXT-883
    # implements: REQ-NEXT-884  # implements: REQ-NEXT-885
    # implements: REQ-NEXT-886  # implements: REQ-NEXT-887
    """Print one risk bucket's requirement list and advice, plus (for
    'unimplemented') the committed-map cross-scan-root hint. `disp` is
    the shared (reqs, show_all, top_n) display context every bucket
    printer takes."""
    ...
    if sig == "unimplemented" and reqs_dir:
        ...
        if recorded:
            ...
            print(("  note: the committed _map.json records member(s) "
                   "for {} of these "
                   "(e.g. {} <- {}) that this scan did not reach — if "
                   "they live outside "
...
def _print_untagged_bucket(untagged, show_all, top_n):
    # implements: ARCH-NEXT-013  # implements: REQ-NEXT-883
    # implements: REQ-NEXT-884
    # implements: REQ-NEXT-885  # implements: REQ-NEXT-886
    # implements: REQ-NEXT-887  # implements: REQ-NEXTUNTAGGED-1050
...
def _print_oversize_bucket(oversize, disp):
    ...
    print(
        "  -> A requirement with more than {} acceptance criteria "
        "covering disjoint "
        "behaviors is a split candidate. Author two requirements, "
        "each with its own "
        "contract.\n"
...
def _print_redundant_bucket(redundant, disp):
    ...
    for g in shown_r:
        print("  {}   identical contract   {}".format(
            ", ".join(g), _req_file(reqs, g[0])))
    if not show_all and len(redundant) > top_n:
        print("  ... {} more — run `reqmap.py gate --risk --all`"
              .format(len(redundant) - top_n))
    print(
        "  -> {} requirement(s) state an obligation another already "
        "states, word for "
        "word. Fold each group into one and re-point the tags, or "
        "make the contracts "
        "say different things. Exact matches only — run `reqmap.py "
        "ask --dupes` for the "
...
def cmd_next(ws, show_all=False, top_n=3):
    ...
    # implements: REQ-NEXT-885  # implements: REQ-NEXT-886
    # implements: REQ-NEXT-887
    """Terminal 'what should I do next': a focused, counted worklist
    over the same `_risk_signals` + `RISK_ADVICE` that drive the Risk
    tab. Prints a progress header, leads with the most-urgent bucket,
    shows the top few per bucket (the extract REVIEW-flagged ones
    first), and collapses the rest behind --all. Each item names the
    requirement file to open. Also surfaces scannable files that carry
    no membership tag (untagged bucket) and, last, the gaps in the
    plan itself (Plan bucket). Read-only, always exit 0."""
    ...
    # oversize or redundant requirement is NOT "nothing pending". The
    # plan's gaps join them for the same reason: a corpus in perfect
...
def _member_roles(members):
    """Roles of a node's members, tolerant of both member shapes in
    play: the raw scan tuples (role, file, line) used by cmd_check and
    the {role, loc} dicts attached to map data nodes."""
    roles = []
    for m in members or []:
...
def _risk_signals(node):
    signals = []
    # 'unimplemented' must mirror the gate, which errors when an
    # ENFORCED requirement has no `implements:` member (a
    # `tested-by`-only member must not satisfy it). Keying on the
    # implements ROLE (not raw member-list emptiness) keeps next/show/
    # the Risk map agreeing with `check`. A `layer: need` is
    # satisfied-by other requirements, not implemented by code, so the
    # gate exempts it (ARCH-TRACE-020) — mirror that here, else the
    # Risk/Problems views flag a passing gate as failing.
    roles = _member_roles(node.get("members"))
    if (node["status"] in ENFORCED and "implements" not in roles
            and not _impl_exempt(node)):
        signals.append("unimplemented")
    if node["status"] in ("draft", "baseline"):
        signals.append("unreviewed")
    # implemented-but-untested: has hand-written code linked but no
    # acceptance test. Gated on an implements member so not-yet-built
    # drafts (already 'unreviewed') are not double-flagged. Opt out
    # per requirement with `test_exempt: <reason>`.
    if ("implements" in roles and "tested-by" not in roles
            and not node.get("test_exempt")):
        signals.append("untested")
    # open verify-intent questions reconstructed from code — surface
    # them on the map, not just in the detail panel / _findings.md.
    # Mirror collect_findings: a "None —" placeholder bullet is not an
    # open finding. A *draft* is suppressed here: its intent questions
    # are subsumed by 'unreviewed' (the whole draft is unreviewed),
    # and every auto-extracted draft carries a template verify TODO —
    # flagging both would double-count every draft. Re-surfaces once
    # promoted past draft. This rule lives in the shared signal source
    # so `next` and the Risk tab agree.
