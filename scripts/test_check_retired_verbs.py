...
"""
# tested-by: REQ-SELFGATE-1011 @unit  # tested-by: ARCH-SELFGATE-039 @unit
import io
import os
import sys
import unittest
...
class LiveSurface(unittest.TestCase):
    ...
    def test_reads_the_parser_including_a_digit_and_a_global(self):  # verifies: REQ-SELFGATE-1011#CASE-1
        flags = C.live_flags()
    ...
    def test_unreadable_engine_fails_open(self):  # verifies: REQ-SELFGATE-1011#CASE-6
...
class RetiredVerbs(unittest.TestCase):
    def test_bare_undelimited_invocation_is_caught(self):  # verifies: REQ-SELFGATE-1011#CASE-2
    ...
    def test_a_live_verb_is_not_a_finding(self):  # verifies: REQ-SELFGATE-1011#CASE-3
        self.assertEqual(scan("run `python scripts/reqmap.py gate --code ..`"), [])

    def test_a_word_that_is_not_a_retired_verb_is_ignored(self):  # verifies: REQ-SELFGATE-1011#CASE-3
...
class RetiredFlags(unittest.TestCase):
    def test_a_flag_the_parser_does_not_accept_is_caught(self):  # verifies: REQ-SELFGATE-1011#CASE-2
    ...
    def test_flags_of_a_second_command_on_the_line_are_not_attributed(self):  # verifies: REQ-SELFGATE-1011#CASE-5
    ...
    def test_flags_after_a_shell_separator_are_not_attributed(self):  # verifies: REQ-SELFGATE-1011#CASE-5
...
class RemovalNotes(unittest.TestCase):
    ...
    def test_english(self):  # verifies: REQ-SELFGATE-1011#CASE-4
        self.assertEqual(scan("`reqmap.py map` no longer exists — use `sync`"), [])

    def test_romanian(self):  # verifies: REQ-SELFGATE-1011#CASE-4
...
class ThisRepo(unittest.TestCase):
    def test_no_instruction_in_this_repo_names_a_retired_name(self):  # verifies: REQ-SELFGATE-1011#CASE-2
        buf = io.StringIO()
    ...
    def test_a_missing_extra_root_is_an_error_not_a_pass(self):  # verifies: REQ-SELFGATE-1011#CASE-6
        buf = io.StringIO()
