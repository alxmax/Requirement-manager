...
python scripts/reqmap.py mcp [--allow-writes]   # serve the engine to an AI assistant over MCP (stdio): 15 tools named for questions, each one reqmap invocation in a fresh process; read-only unless --allow-writes (ADR-0043). init writes .mcp.json and .vscode/mcp.json
