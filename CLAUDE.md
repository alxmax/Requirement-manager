...
**`level: architecture` was promoted into `level: system`, also since 2026-09-03** (`docs/adr/0024`). This corpus now carries 71 `level: system` requirements — the 9 original `layer: need` stakeholder requirements (`SYS-*`, fan-out 5–10) plus the 62 former `layer: bus`/`feature` architecture requirements (`ARCH-*`, fan-out up to 32) — 0 at `level: architecture`, and 573 at `level: code`. **The `ARCH-` prefix on a promoted id is now purely historical** — those ids' `level:` reads `system`, not `architecture`; only their filename/id prefix still says otherwise, per the reading-convenience rule above. `level: system` therefore spans two populations distinguished by `layer:`, not by `level:`: read `layer:` to tell a root stakeholder need from a grouping node. The `architecture` level itself is still valid (`VALID_LEVEL`, `LINT_FANOUT_BANDS["architecture"]`) and unpopulated in this corpus by design, not by accident — a future requirement, or a consumer repo, may still use it. `fan-out`'s `system` ceiling is fifty (`docs/adr/0023` set it to ten for the old 9-node-only population; `docs/adr/0024` raised it once the promoted population joined).
