...
**Per-repo configuration:** `requirements/_config.json` overrides the constants named in `CONFIG_KEYS` (`LINT_AC_MAX`, `SIMILAR_THRESHOLD`, `ORPHAN_CODE_MIN_LOC`, `LINT_FANOUT_BANDS`, ...) plus `extra_code_exts`; validated at CLI startup; malformed JSON, unknown active keys and mistyped values stop the command before writes. Library helpers retain their fallback defaults and accept a diagnostic collector (`ARCH-CONFIG-060`), except a key v8.2.0 retired (`DESIGN_*`, `LANGUAGE`), which is skipped in silence (ADR-0047). This repo sets `MAP_PROFILE: compact` and `MAP_LOCALES: []` to keep derived maps small.
