...
python scripts/reqmap.py mcp [--allow-writes]   # serve the engine to an AI assistant over MCP (stdio): 14 tools named for questions, each one reqmap invocation in a fresh process, plus each requirement and the map as resources; read-only unless --allow-writes (ADR-0043). init writes .mcp.json and .vscode/mcp.json
