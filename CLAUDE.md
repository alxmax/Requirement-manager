...
**Per-repo configuration:** `requirements/_config.json` overrides the constants named in `CONFIG_KEYS` (`LINT_AC_MAX`, `SIMILAR_THRESHOLD`, `ORPHAN_CODE_MIN_LOC`, `LINT_FANOUT_BANDS`, ...) plus `extra_code_exts`; read fail-open at startup by `apply_config(load_config(...))`, an unknown or mistyped key is reported on stderr and skipped (`ARCH-CONFIG-060`), except a key v8.2.0 retired (`DESIGN_*`, `LANGUAGE`), which is skipped in silence (ADR-0047). This repo ships none.
