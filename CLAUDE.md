...
python scripts/reqmap.py gate --i18n [--json]      # translations the configured LANGUAGE (en|ro|both, in _config.json) expects and does not have; --json hands over source fields + cache key
...
The CLI has six verbs — `init`, `new`, `gate`, `sync`, `clarify`, and `mcp`, the server (ADR-0043) (`reqmap.py --help`); everything else is a flag of one of them.
...
`--search`, `--dupes`, `--review`, `clarify`) is unaffected either way and can be run with or
...
- `layer: feature` — compose the bus via `depends_on`. One per capability — a verb, or a flag of one (`gate --dupes`, `sync --release`, …); `ls plugin/requirements/` is the live list.
