...
# `new` and `new --from-todo` are deprecated and removed in v8.0.0 (ADR-0045)
...
python scripts/reqmap.py mcp [--allow-writes]   # serve the engine to an AI assistant over MCP (stdio): 15 tools named for questions, each one reqmap invocation in a fresh process, plus each requirement and the map as resources; read-only unless --allow-writes (ADR-0043). init writes .mcp.json and .vscode/mcp.json
...
python scripts/reqmap.py ask --i18n [--json]      # translations the configured LANGUAGE (en|ro|both, in _config.json) expects and does not have; --json hands over source fields + cache key
...
python scripts/reqmap.py ask --design [--json]     # advisory design review of the repo's code (Python via ast; brace languages via heuristics): the four OOP pillars, one C&K per-class metric (RFC, Python only), plus house standards; read-only, exit 0, never the gate; thresholds in _config.json
...
The CLI has seven verbs — `init`, `new`, `gate`, `sync`, `clarify`, `ask` (every read-only question that is not the verdict, ADR-0044), and `mcp`, the server (ADR-0043) (`reqmap.py --help`); everything else is a flag of one of them. Until v8.0.0 `gate` still accepts `ask`'s flags (`--search`, `--dupes`, `--design`, `--review`, `--i18n`, `--top`, `--threshold`) with one migration line on stderr.
...
**Design decisions live in `docs/adr/`** (45 records, index at `docs/adr/README.md`) — the engine's shape (`0014` kept it one file, `0033` priced the split, `0035` made it a package at the maintainer's direction), the error-versus-warning split, the drift-baseline shape, the V-model (`0007` parked it, `0019` supersedes it by adopting the left arm warn-only), and four rejected proposals. Read the relevant record before proposing a change that reverses one; each names the evidence it was decided on and its revisit condition. A decision that changes gains a NEW record superseding the old one — never an edit to the old one.

**The engine is a package behind a thin CLI** (`docs/adr/0035`, plugin `v7.0.0`): `plugin/scripts/reqmap.py` is the command line only — parser, dispatch, the Python floor, and the flat namespace `import reqmap` has always offered (a module-level `__getattr__` looks any engine name up across the package, so `R.LINT_AC_MAX` and `R._acc_blocks` still work) — and `plugin/scripts/reqmap_engine/` holds the logic, 59 modules beside `__init__.py`, one per capability, stdlib only, and the whole package passes `ask --design`'s own review: no file over 500 lines or 30 definitions, no line over 100 columns, no function over 80 lines or nested past 4 levels, no prefix family. **It reports exactly TWO findings on itself, and a test asserts that number** (`DocsAreTrue.test_the_engine_reports_the_documented_design_findings`) — this sentence drifted from 3 to 10 unnoticed once, which is the failure this repo exists to prevent. ADR-0035 accepted four; ADR-0041 fixed two of them in code (`cmd_check` takes six parameters, the reason for accepted drift travelling in `accept_drift` itself; `mapcmd`'s freshness helpers take the workspace instead of `reqs`, `reqs_dir` and a root) and kept two. The first kept is `apply_config`'s module write: overriding the module constants IS the configuration design, and rewriting `global` as `globals()[...]` would hide it from the detector without changing what it does. The second is `rules.py`'s 32 top-level definitions: the file IS the gate registry (ADR-0026, one `@gate_rule` function per check), so splitting it to satisfy a count would fragment the single place the rules live — the threshold measures a module doing too many things, and this one does exactly one. Change the count only by changing the code; a new finding is a decision to record here, not a number to edit. A helper extracted from a tagged function carries a copy of the parent's `# implements:` line as the first line of its body — keep that when you extract one, or the member sidecar loses coverage of the moved code. The layering is a bus, imported by name and never upward: `config → model → parse/sections/acceptance/text → tags/scan/orphans → git → locks → (features) → workspace → rules → gate → reqmap.py`. Three rules keep it one: every `_config.json` tunable lives in `config.py` and is read as `cfg.NAME` (a name import would snapshot the default and miss the override); nothing imports upward, with two documented call-time imports (`health._link_sync_errors` → `rules` + `workspace.GateContext`, because the gate's map rule embeds the health record; and `risk._plan_gaps` → `mapdata`, because `mapdata` reads `_risk_signals` from `risk`); and `MAP_ENGINE_VERSION` lives in `reqmap_engine/__init__.py`, which every probe reads first, falling back to `reqmap.py` for a single-file copy seeded before v7. A test that patches an engine name patches the module that looks it up (`R.git._git`, not `R._git`). The two travel together: seeding copies `scripts/reqmap.py` AND `scripts/reqmap_engine/`.
...
**Per-repo configuration:** `requirements/_config.json` overrides the constants named in `CONFIG_KEYS` (`LINT_AC_MAX`, `SIMILAR_THRESHOLD`, `ORPHAN_CODE_MIN_LOC`, `LINT_FANOUT_BANDS`, ...) plus `extra_code_exts` and the `LANGUAGE` enum (`en` | `ro` | `both`, `REQ-TRANSLATE-996`); read fail-open at startup by `apply_config(load_config(...))`, an unknown or mistyped key is reported on stderr and skipped (`ARCH-CONFIG-060`). This repo ships none.
